...
IMPORT STATUS usrUsb2KbdInit (void);
...
#include <edrStub.c>
#include <evdevLibKbdCfg.c>
#include <gfxSplashInit.c>
#include <gfxTiDssInit.c>
...
void usrIosExtraInit (void)
    ...
    xbdBlkDevLibInit (XBD_BLK_DEV_TASK_PRIORITY,                                       XBD_BLK_DEV_TASK_STACK_SIZE,                                       XBD_BLK_DEV_DIRECT_BIO_HANDLING); /* This component provides support for legacy block devices                     that are designed to work the predecessor to XBD: the                     cache block I/O (CBIO) facility. It provides a wrapper XBD                     facility that converts the block I/O driver interface                     based on the BLK_DEV logical block device structure into an                     XBD API-compliant interface.                     NOTE: The Wind River devices that require the                     INCLUDE_XBD_BLK_DEV component in addition to INCLUDE_XBD                     are floppy and TrueFFS (the disk-access emulator for flash)                     drivers. Any third-party device drivers based on the                     BLK_DEV interface also require INCLUDE_XBD_BLK_DEV. The                     Wind River drivers that do not require the                     INCLUDE_XBD_BLK_DEV component are USB block storage, ATA,                     and the XBD RAM disk.                     CAUTION: Depending on the implementation of the driver,                     the INCLUDE_XBD_BLK_DEV component may not properly detect                     media insertion and removal. It may, therefore remove the                     file system when the media is removed, or not instantiate                     a file system when media is inserted. */
    ...
    evdevKbdInit (EVDEV_KBD_TYPE_MATIC_PERIOD); /* This component is required when using keyboard devices. It                     includes the event devices keyboard library. */
    usrUsb2KbdInit ();                  /* USB GEN2 Keyboard Driver Initialization */
    usrUsb2MscInit ();                  /* USB GEN2 Mass Storage Driver Initialization */
    evdevKbdMapUSInit ();               /* US Keyboard Mapping */
...
void usrShowInit (void)
    ...
    semPxShowInit ();                   /* When VxWorks is configured with this component, you                         can use the show( ) routine from the shell (with the C                         interpreter) to display information about a POSIX                         semaphore. The show( ) routine takes the semaphore ID                         as the argument. */
...
void usrNetAppInit (void)
    {
    usrSnmpCfgInit();                   /* Initialize Wind Manage SNMP. */
    }
...
/******************************************************************************
*
* usrUsbPerStageInit - The usb earlier initialization before vxBus init
*/

void usrUsbPerStageInit (void)
    {
...
void usrRoot (char *pMemPoolStart, unsigned memPoolSize)
    ...
    vxDyncIntLibInit();                 /* This component includes the VxBus dynamic interrupt controller                     library, vxbDyncLibInt.  */
    ...
    vxbPciInit ();                      /* The VxBus PCI subsystem library component provides                     platform-independent bus scan and helper routines used by                     PCI host controller drivers and individual function drivers. */
    vxbPciMsiInit ();                   /* The VxBus PCI MSI/MSI-X library provides support routines                     for handling MSI and MSI-X interrupts with PCI devices. */
    ...
    usrRtpInit ();                      /* RTP init routines */
    taskPxLibInit ();                   /* This component provides POSIX thread scheduling                         behavior in RTPs. */
    ...
    usrUsbGroupInit();                  /* The USB group initialization */
    usrToolsInit ();                    /* Software development tools */
    gfxTiDssInit0();                    /* TI DSS frame buffer 0 */
