...
void usrAppInit (void)
{
	...
	/* Delay to let the vxWorks file system mount the SD-Card and
	 * accessing the resources required to run the EtherCAT Master from the SD-Card.
	 */		
	taskDelay(DELAY * sysClkRateGet());
	
	coreDumpRtpFsPathSet(RTP_CORE_DUMP_PATH);
	...
	printf("IMG1.. vxbGpioAlloc status: %d\n", status);
	vxbGpioSetDir(0x69,1);
	printf("IMG1.. vxbGpioSetDir status: %d\n", status);
	
#ifdef CONFIG_CUSTOM_SC_AM5728
	status = enableSATA();
	if(OK != status)
		printf("[ Error ]: Enabling SATA failed !\n");
#endif
	
	status = initCpsw1();
	if(OK != status)
		printf("[ Error ]: CPSW2 initialization failed !\n");
	
	/* TODO: add application specific code here */
	/* Load link layer and starts Master stack */
	//TODO: Temporarily DKM is loaded after launching RTP
	//		from usrRtpAppInit.c to enable autoboot
	//loadApplicationModule(KERNEL);
}
