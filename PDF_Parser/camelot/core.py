...
class TableList(object):
    ...
    def _write_file(self, f=None, normal=True, **kwargs):
        dirname = kwargs.get("dirname")
        root = kwargs.get("root")
        ext = kwargs.get("ext")
        for table in self._tables:
            if normal:
                controller.conversion_tracking(table.page, len(self._tables))
    ...
    def export(self, path, f="csv", compress=False, normal=True):
        ...
        if f in ["csv", "json", "html"]:
            self._write_file(f=f, normal=normal, **kwargs)
            if compress:
