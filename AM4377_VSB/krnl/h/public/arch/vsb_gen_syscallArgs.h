/* Aligned for arm - no alignment needed */
...
struct AuxClkConnectScArgs 
	{
	INT32 task_id;
	};

struct AuxClkRateSetScArgs 
	{
	INT32 ticksPerSecond;
	};

struct GpioAllocScArgs 
	{
	UINT32 id;
	};

struct GpioGetValueScArgs 
	{
	UINT32 id;
	};

struct GpioSetValueScArgs 
	{
	UINT32 id;
	UINT32 value;
	};

struct GpioSetDirScArgs 
	{
	UINT32 id;
	UINT32 value;
	};
...
struct EhrPwmRegisterISRScArgs 
	{
...
struct EhrPwmSetAQSFRCScArgs 
	{
	UINT32 value;
	UINT8 inst;
	};

struct EhrPwmSetTbClkScArgs 
	{
	UINT32 tb;
	UINT8 ehrpwm_inst;
	};

struct EhrPwmSetTbAQConfAScArgs 
	{
	UINT32 tb;
	UINT8 ehrpwm_inst;
	};

struct EhrPwmSetTbAQConfBScArgs 
	{
	UINT32 tb;
	UINT8 ehrpwm_inst;
	};

struct EhrPwmSetTbPWMCfgScArgs 
	{
	UINT32 freq;
	UINT32 dir;
	UINT8 ehrpwm_inst;
...
struct EqepSetMaxCountScArgs 
	{
	INT32 eqep;
	INT32 count;
	};

struct EqepGetPosCountScArgs 
	{
	INT32 eqep;
	};

struct McSpiTransScArgs 
	{
...
struct MsDelayScArgs 
	{
	INT32 delayTime;
	};

struct GpioGetDirScArgs 
	{
	UINT32 id;
	};

struct GpioFreeScArgs 
	{
	UINT32 id;
	};

struct logCRITScArgs 
	{
	UINT8 *logData;
	UINT32 lineNo;
...
struct Am5728EhrPwmRegisterISRScArgs 
	{
	EHRPWMDRV_IRQ_HANDLER irqHandler;
	void * ehrpwmHandlerArgs;
	UINT8 ehrpwm_inst;
...
struct Am5728EhrPwmSetAQSFRCScArgs 
	{
	UINT32 value;
	UINT8 inst;
	};

struct Am5728EhrPwmSetTbClkScArgs 
	{
	UINT32 tb;
	UINT8 ehrpwm_inst;
	};

struct Am5728EhrPwmSetTbAQConfAScArgs 
	{
	UINT32 tb;
	UINT8 ehrpwm_inst;
	};

struct Am5728EhrPwmSetTbAQConfBScArgs 
	{
	UINT32 tb;
	UINT8 ehrpwm_inst;
	};

struct Am5728EhrPwmSetTbPWMCfgScArgs 
	{
	UINT32 freq;
	UINT32 dir;
	UINT8 ehrpwm_inst;
...
struct QSPIFlashEraseScArgs 
	{
	unsigned int offset;
	unsigned int length;
	};

struct QSPIFlashWriteScArgs 
	{
	unsigned int offset;
	unsigned int length;
	unsigned int src;
...
struct QspiLibBlockEraseScArgs 
	{
	unsigned int block;
	};

struct QspiLibWriteScArgs 
	{
	unsigned int dstOffsetAddr;
	unsigned int srcAddr;
	unsigned int length;
	};

struct QspiLibReadScArgs 
	{
	unsigned int srcOffsetAddr;
	unsigned int dstAddr;
	unsigned int length;
	};

struct SioPollInputScArgs 
	{
	SIO_CHAN * pChan;
	char *pChar;
	};

struct SioPollOutputScArgs 
	{
	SIO_CHAN * pChan;
	char outChar;
	};

struct GpioIntConnectScArgs 
	{
	UINT32 id;
	VOIDFUNCPTR pIsr;
	void *ptr;
...
struct EmmcExtCsdDecodeScArgs 
	{
	EST_LIFE * emmc_est_life;
	};

struct GetRTClowPwrScArgs 
	{
	RTCtimerStr * pTime;
	};
...
#if (defined(_WRS_CONFIG_USER_MANAGEMENT))
struct userPasswordUpdateScArgs 
	{
	const char * loginName;
	const char * oldClearPassword;
	const char * newClearPassword;
	};
#endif

#if (defined(_WRS_CONFIG_USER_MANAGEMENT))
struct _getpwuidScArgs 
	{
	int userId;
	struct passwd * pwd;
	char * buffer;
