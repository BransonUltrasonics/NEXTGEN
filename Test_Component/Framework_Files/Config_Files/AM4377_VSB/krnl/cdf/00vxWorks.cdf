...
Selection SELECT_EDR_POLICY {
        NAME            ED&R fatal policy handler selection
        SYNOPSIS        Selects a fatal error handling policy. This allows the \
                        user to select the kernel-provided default fatal error \
                        handling policy or a custom user-defined fatal error \
                        handling policy that the user can implement.
        COUNT           1-1
...
Component INCLUDE_EDR_STUB {
        NAME            ED&R stub
        SYNOPSIS        Error detection and reporting (ED&R) error-injection stub.
...
Component INCLUDE_EDR_DEFAULT_POLICY {
        NAME            Default fatal error handler policy
...
Component INCLUDE_EDR_CUSTOM_POLICY {
        NAME            Custom fatal error handler policy
        SYNOPSIS        Adds a custom fatal error handler policy. When this \
                        component is selected, the fatal errors are no longer \
                        handled by the kernel. The kernel default fatal policy \
                        handler routines are replaced by a similar set of stub \
                        fatal policy handler routines available to the user in \
                        the edrCustomPolicy.c file in the VxWorks image prject \
                        (VIP). It now becomes the user's responsibility to add \
                        in code that handles the fatal errors. \
                        Note: The features provided by the components \
                        INCLUDE_CORE_DUMP, INCLUDE_CORE_DUMP_RTP, and \
                        INCLUDE_INCLUDE_STOP_MODE_AGENT will no longer be \
                        available by default. If these features are desired, \
...
Component INCLUDE_EDR_POLICY_HOOKS {
        ...
        SYNOPSIS        Policy hook routine support for error detection and \
                        reporting.
...
Component INCLUDE_SYS_START {
        ...
        SYNOPSIS        Clears BSS and set up the vector table base address.
...
Folder  FOLDER_POSIX {
        ...
        SYNOPSIS        This folder contains components and parameters that \
                        can be used to include and configure different POSIX \
...
Component INCLUDE_POSIX_USER_GROUP {
        ...
        SYNOPSIS        This component adds support for POSIX user and group \
...
Component INCLUDE_POSIX_UMASK {
        ...
        SYNOPSIS       This component adds support for the POSIX file mode \
                       creation mask in the kernel environment.
        MODULES        umaskLib.o
...
Component INCLUDE_POSIX_MQ {
        ...
        SYNOPSIS        This is the primary component for support of the \
                        message queue POSIX facility for both kernels and \
...
Component INCLUDE_POSIX_SCHED {
        ...
        SYNOPSIS        Provides kernel-only emulation of the process \
                        scheduling API, no scheduler.
...
Component INCLUDE_POSIX_SEM {
        NAME            Semaphores
        SYNOPSIS        This component provides support for the POSIX semaphore routines \
...
Component INCLUDE_POSIX_SIGNALS {
        ...
        SYNOPSIS        This component automatically initializes POSIX queued \
                        ...
                        routine allocates buffers for use by sigqueue( ), \
...
Parameter HIGH_RES_POSIX_CLOCK {
        NAME            High resolution POSIX clock
...
Parameter POSIX_CLOCK_TIMER {
        NAME            Timer to be used with the POSIX clock component
        SYNOPSIS        Specifies the timer to use in association with \
                        HIGH_RES_POSIX_CLOCK. If left NULL, the next available \
                        timer with the highest frequency is used.
        TYPE            char *
        DEFAULT         NULL
}
...
Component INCLUDE_POSIX_TIMER_SHOW {
        ...
        SYNOPSIS        This component includes a utility that allows the user to display \
                        ...
                        routine which shows information on a specified timer. It \
...
Component INCLUDE_HW_FP_SHOW {
        ...
        SYNOPSIS        This component includes the fppArchShow library. This \
                        library provides the fppCtxShow() routine that prints \
...
Component INCLUDE_MSG_Q_SHOW {
        ...
        SYNOPSIS        This component includes the msgQShow library in your \
                        project. This library provides routines to show \
                        message queue statistics, such as the task queuing \
                        method, messages queued, and receivers blocked. The \
                        msgQShowInit() routine links the message queue show \
                        facility into the VxWorks system. It is called \
                        automatically when you include this component in your \
                        project. The msgQShow() routine displays information \
                        about message queues, such as the task queuing method, \
                        messages queued, and receivers blocked.
        MODULES         msgQShow.o
...
Component INCLUDE_SEM_SHOW {
        ...
        SYNOPSIS        This component includes the semShow library in your \
                        ...
                        Routines in this library are meant to be used as \
...
Component INCLUDE_CONDVAR_SHOW {
        ...
        SYNOPSIS        This component provides the conVarShow() routine. This \
                        routine can be used to show information about a \
...
Component INCLUDE_TASK_HOOKS_SHOW {
        ...
        SYNOPSIS        This component provides support for the task hook show routines: \
                        taskHookShowInit() initializes the task hook show facility; \
                        taskCreateHookShow() shows the list of task create routines; \
                        taskDeleteHookShow() shows the list of task delete routines; \
                        taskSwitchHookShow() shows the list of task switch routines.
...
Component INCLUDE_WATCHDOGS_SHOW {
        ...
        SYNOPSIS        This component provides the wdShow library. This \
                        library provides routines to show watchdog statistics, \
                        such as watchdog activity, a watchdog routine, and so \
                        forth. The wdShowInit() routine links the watchdog \
                        show facility into the VxWorks system. It is called \
                        automatically when this component is included in your \
                        project. The wdShow() routine shows information about \
                        a watchdog.
        MODULES         wdShow.o
        INIT_RTN        wdShowInit ();
...
Component INCLUDE_TASK_SHOW {
        ...
        SYNOPSIS        This component provides routines to show task-related \
                        ...
                        and taskRegsShow() routines are used to display task \
                        ...
                        routines should therefore be viewed as a snapshot of \
...
Component INCLUDE_MEM_SHOW {
        ...
        SYNOPSIS        This component includes the memShow library. This \
                        ...
                        routines. The memShow() routine shows blocks and \
                        statistics for the current heap partition. The \
                        memPartShow() routine shows available and allocated \
...
Component INCLUDE_CLASS_SHOW {
        ...
        SYNOPSIS        This component provides the classShow library which \
                        provides routines to show class-related information.
        MODULES         classShow.o
...
Component INCLUDE_POSIX_MQ_SHOW {
        ...
        SYNOPSIS        This component provides the mqPxShow library. This \
                        ...
                        the call is made. The mqPxShow() routine displays \
                        message queue internals. The mqPxShowInit() routine \
...
Component INCLUDE_POSIX_SEM_SHOW {
        ...
        SYNOPSIS        When VxWorks is configured with this component, you \
                        can use the show( ) routine from the shell (with the C \
                        interpreter) to display information about a POSIX \
                        semaphore. The show( ) routine takes the semaphore ID \
                        as the argument.
        MODULES         semPxShow.o
...
Component INCLUDE_ISR_SHOW {
        ...
        SYNOPSIS        This component provides support for the isrShow() \
                        routine. It shows information about an ISR object.
...
Component INCLUDE_HANDLE_SHOW {
        ...
        SYNOPSIS        This component provides the handleShow library. This \
                        library provides routines to show handle information.
...
Component INCLUDE_OBJECT_SHOW {
        ...
        SYNOPSIS        This component includes the objShow library. This \
                        library provides the objShowAll() routine to show the \
                        ...
                        objShow() routine is invoked to display the \
                        class-specific information. The objHandleShow() \
                        routine shows information on the object referenced by \
                        an object handle. The objHandleTblShow( ) routine \
...
Component INCLUDE_USER_APPL {
        ...
        SYNOPSIS        Calls usrAppInit() (in your usrAppInit.c project file) \
                        after startup.
...
Component INCLUDE_USER_PRE_KERNEL_APPL_INIT {
        ...
        SYNOPSIS        Calls usrPreKernelAppInit() (in your usrAppInit.c \
                        project file).
...
Component INCLUDE_USER_POST_KERNEL_APPL_INIT {
        ...
        SYNOPSIS        Calls usrPostKernelAppInit() (in your usrAppInit.c \
                        project file).
...
Component INCLUDE_USER_PRE_NETWORK_APPL_INIT {
        ...
        SYNOPSIS        Calls usrPreNetworkAppInit() (in your usrAppInit.c \
                        project file).
        INIT_RTN        usrPreNetworkAppInit ();
}

Component INCLUDE_BANNER {
        NAME            VxWorks banner containing version and creation date
        SYNOPSIS        Displays the Wind River banner on startup.
...
Folder  FOLDER_KERNEL {
        NAME            Kernel
        SYNOPSIS        This folder contains different components and \
...
Component INCLUDE_REBOOT_HOOKS {
        ...
        SYNOPSIS        This component provides support for reboot hooks with \
                        the rebootHookLib library. When the reboot() routine \
                        ...
                        routines that can block. For example, calling free() \
                        ...
                        in an undefined state. The rebootHookAdd( ) routine \
                        adds the specified routine to a list of routines to be \
...
Folder  FOLDER_EDR {
        NAME            ED&R
        SYNOPSIS        This folder contains components and parameters \
                        concerning error detection and reporting.
...
Component INCLUDE_EXC_TASK {
        ...
        SYNOPSIS        This component includes the excLib facility. This \
                        ...
                        following routines: excInit() which initializes the \
                        ...
                        routine to be called with exceptions.
#ifndef _WRS_CONFIG_CERT_KERNEL_OBJECT
        MODULES         excLib.o
#else
        _REQUIRES INCLUDE_KERNEL
...
Component INCLUDE_ISR_OBJECTS {
        ...
        SYNOPSIS        Objects describing interrupt service routines (ISRs).
        MODULES         isrLib.o
...
Parameter FUNCPTR_OBJ_MEMALLOC_RTN {
        NAME            Function pointer to objMemAllocRtn
        SYNOPSIS        Specifies the function pointer to the \
                        objMemAllocRtn( ) routine.
        TYPE            void *
        DEFAULT         (INCLUDE_MEM_ALLOT)::(objMemAllot) memPartAlignedAlloc
}

Parameter FUNCPTR_OBJ_MEMFREE_RTN {
        NAME            Function pointer to objMemFreeRtn
        SYNOPSIS        Specifies the function pointer to the \
                        objMemFreeRtn( ) routine.
        TYPE            FUNCPTR
...
Parameter OBJ_MEM_POOL_ID {
        ...
        SYNOPSIS        Specifies the object memory pool ID.
        TYPE            void *
...
Component INCLUDE_OBJ_INFO {
        ...
        SYNOPSIS        This component provides object management routines \
                        that require lookup in a list of objects, such as \
                        the objNameToId() routine.
#ifndef _WRS_CONFIG_CERT_KERNEL_OBJECT
        MODULES         objInfo.o
#else
        _REQUIRES INCLUDE_KERNEL
...
Component INCLUDE_OBJ_OWNERSHIP {
        ...
        SYNOPSIS        This component provides the objOwnerLib library which \
                        includes object ownership routines.
#ifndef _WRS_CONFIG_CERT_KERNEL_OBJECT
...
Parameter VXEVENTS_OPTIONS {
        ...
        SYNOPSIS        For the list of supported options and a more detailed \
                        description, see the reference information for eventLib.
        TYPE            UINT
...
Component INCLUDE_MSG_Q {
        ...
        SYNOPSIS        This component provides message queue support using the \
                        msgQLib library. This library contains routines for \
                        ...
                        interrupt service routine can send messages to a \
...
Component INCLUDE_MSG_Q_CREATE_DELETE {
        ...
        SYNOPSIS        This component provides the msgQCreateLib library. This \
                        library contains routines for creating and deleting \
                        message queues. The routines to initialize and operate \
...
Component INCLUDE_MSG_Q_EVENTS {
        ...
        SYNOPSIS        This library is an extension to eventLib, the VxWorks \
                        events library. Its purpose is to support events for \
                        message queues. The routines in this library are used to \
                        control registration of tasks on a message queue. The \
                        msgQEvStart() routine registers a task and starts the \
                        notification process. The msgQEvStop() routine \
                        un-registers the task, which stops the notification \
                        mechanism. When a task is registered and a message \
                        arrives on the queue, the events specified are sent \
                        to that task, on the condition that no other task is \
                        pending on that message queue. However, if a \
                        msgQReceive() is to be done afterwards to get the \
                        message, there is no guarantee that it will still be \
                        available.
#ifndef _WRS_CONFIG_CERT_KERNEL_OBJECT
        MODULES         msgQEvLib.o
#else
...
Component INCLUDE_SEM_LIB {
        ...
        SYNOPSIS        Semaphores are the basis for synchronization and mutual \
                        ...
                        semaphore routines common to all VxWorks semaphore types. \
                        ...
                        operations, including routines for semaphore control, \
...
Component INCLUDE_SEM_DELETE {
        ...
        SYNOPSIS        This component provides the semDeleteLib library that \
                        contains routines that terminate and delete semaphores. \
                        ...
                        routines that were pended return ERROR. Take care when \
...
Component INCLUDE_SEM_BINARY {
        ...
        SYNOPSIS        This library provides the interface to VxWorks binary \
                        semaphores. Binary semaphores are the most versatile, \
                        efficient, and conceptually simple type of semaphore. \
                        They can be used to: control mutually exclusive access \
                        to shared devices or data structures, or synchronize \
                        multiple tasks, or task-level and interrupt-level \
                        processes. Binary semaphores form the foundation of \
                        numerous VxWorks facilities.
#ifndef _WRS_CONFIG_CERT_KERNEL_OBJECT
        MODULES         semBLib.o
#else
        _REQUIRES INCLUDE_KERNEL
...
Component INCLUDE_SEM_MUTEX {
        ...
        SYNOPSIS        This library provides the interface to VxWorks \
...
Component INCLUDE_SEM_READ_WRITE {
        ...
        SYNOPSIS        This component provides the semRWLib and windSemRWLib \
                        ...
                        implementation of the windSemRWDelete() routine. A call \
                        ...
                        any and all owners have their priority inheritence \
                        information updated.
...
Component INCLUDE_CONDVAR {
        ...
        SYNOPSIS        Condition variables allow tasks to wait on both an \
                        ...
                        variable routine is callable from an ISR. For detailed \
                        descriptions of the routines provided in this \
...
Component INCLUDE_SEM_BINARY_CREATE {
        ...
        SYNOPSIS        This component provides the semBCreateLib which \
                        implements semBCreate() - the routine that creates \
                        binary semaphores.
#ifndef _WRS_CONFIG_CERT_KERNEL_OBJECT
...
Component INCLUDE_SEM_COUNTING_CREATE {
        ...
        SYNOPSIS        This component provides semCCreate(), the routine that \
...
Component INCLUDE_SEM_READ_WRITE_CREATE {
        ...
        SYNOPSIS        This component provides the semRWCreateLib library which \
                        implements semRWCreate(), the routine that creates \
...
Component INCLUDE_SEM_MUTEX_CREATE {
        ...
        SYNOPSIS        This component provides the semMCreate() routine. This \
                        routine creates mutual-exclusion semaphores.
#ifndef _WRS_CONFIG_CERT_KERNEL_OBJECT
...
Component INCLUDE_SEM_INFO {
        NAME            Semaphore information routines
        SYNOPSIS        This library provides routines to retrieve information \
                        about a semaphore. The semInfo() routine returns \
                        information about tasks blocked on the semaphore. Given \
                        a SEM_INFO structure, the semInfoGet() routine returns \
...
Component INCLUDE_SEM_EXCHANGE {
        ...
        SYNOPSIS        This component provides the semExchange library. This \
                        library provides the semExchange() routine. This \
                        routine atomically gives one semaphore and takes \
...
Component INCLUDE_TASK_INFO {
        NAME            Task information routines
        SYNOPSIS        This component provides the taskInfo library. This \
                        ...
                        taskInfoGet(), and taskIdListGet() routines are used \
                        to obtain task information. Three routines -- \
                        ...
                        Information provided by these routines should therefore \
                        ...
                        routines are provided by taskLib. Higher-level task \
...
Component INCLUDE_TASK_LIST {
        ...
        SYNOPSIS        This component provides the task utility routines that \
...
Parameter VX_MAX_TASK_CREATE_RTNS {
        NAME            Maximum task create/delete hooks
        SYNOPSIS        Specifies the maximum size of the task create and delete \
                        hook tables. By default, it is set to 40.
        TYPE            UINT
        DEFAULT         40
}
...
Component INCLUDE_TASK_ROTATE {
        ...
        SYNOPSIS        Rotates the task ready queue for a given priority.
...
Component INCLUDE_WATCHDOGS {
        ...
        SYNOPSIS        This library provides a general watchdog timer facility. \
                        Any task can create a watchdog timer and use it to run a \
                        specified routine in the context of the system-clock ISR, \
                        after a specified delay. Once a timer has been created \
                        with wdCreate(), it can be started with wdStart(). The \
                        wdStart() routine specifies what routine to run, a \
                        parameter for that routine, and the amount of time (in \
                        ticks) before the routine is to be called. The timeout \
                        ...
                        is called first to cancel the timer, the timeout routine \
                        is invoked with the parameter specified in the wdStart() \
                        call. The timeout routine is invoked whether the task \
                        which started the watchdog is running, suspended, or \
                        deleted. The timeout routine executes only once per \
                        ...
                        routine is invoked at interrupt level, rather than in \
                        the context of the task. Thus, there are restrictions \
                        on what the routine can do. Watchdog routines are \
                        ...
                        routines like printf(). Note: Watchdog routine invocation \
                        can be deferred. As such isrIdCurrent is either a valid \
...
Component INCLUDE_WATCHDOGS_CREATE_DELETE {
        ...
        SYNOPSIS        This component provides wdCreateLib. This library \
                        provides creation and deletion routines for the \
                        watchdog facility. The routines to initialize and \
...
Component INCLUDE_HOOKS {
        ...
        SYNOPSIS        Allows adding and removing of hook functions.
#ifndef _WRS_CONFIG_CERT_KERNEL_OBJECT
        MODULES         hookLib.o
#else
        _REQUIRES INCLUDE_KERNEL
...
Component INCLUDE_HOOK_SHOW {
        ...
        SYNOPSIS        This is the show component for hook function tables.
        MODULES         hookShow.o
        REQUIRES        INCLUDE_HOOKS
        LINK_SYMS       hookShow
...
Selection SELECT_MEM_ALLOC {
        ...
        SYNOPSIS        Choose between a basic memory allocator and a memory \
                        allot allocator.
        COUNT           1-1
...
Component INCLUDE_MEM_MGR_FULL {
        ...
        SYNOPSIS        This component provides the memLib library. This \
                        ...
                        ANSI allocation routines and enhanced memory management \
                        features, including error handling and aligned \
                        allocation. Most of the memLib routines are simple \
...
Component INCLUDE_MEM_MGR_INFO {
        NAME            Memory allocator information routines
        SYNOPSIS        This component provides the memInfo library. This library \
                        provides routines for obtaining information about a \
...
Component INCLUDE_KERNEL_PROXIMITY_HEAP {
        ...
        SYNOPSIS        This component provides the kProxHeapLib library. \
                        This library provides the routine that initializes the \
...
Component INCLUDE_KERNEL_COMMON_HEAP {
        ...
        SYNOPSIS        The kernel common heap is the memory partition used by \
                        the kernel and kernel applications for dynamic memory \
                        allocation. Physical memory is allocated from the global \
                        RAM pool, virtual memory is allocated from the kernel \
                        virtual memory pool region. It is managed using the \
...
Parameter KERNEL_COMMON_HEAP_INIT_SIZE {
        ...
        SYNOPSIS        The initial size of the heap is defined by this \
                        parameter. It should not be set to a value larger \
                        than the available largest physically contiguous block \
                        of RAM. Take into account the size of a page and other \
...
Parameter KERNEL_COMMON_HEAP_INCR_SIZE {
        ...
        SYNOPSIS        The kernel common heap supports auto-growth. When \
                        the current heap content cannot satisfy the requested \
                        allocation, the operating system automatically adds \
                        more memory which is allocated from the kernel virtual \
                        memory pool region and the global RAM pool. Growth is \
                        subject to available free RAM in the pool. The amount \
                        of heap growth is a multiple of this parameter. \
                        Setting this parameter to zero disables auto-growth.
        TYPE            ULONG
        DEFAULT         0x200000
}
...
Component INCLUDE_SIGNALS {
        ...
        SYNOPSIS        This component provides the sigLib library. This \
                        ...
                        kill() or raise() routines, and will be caught by any \
                        task in that RTP which has unmasked that signal. \
                        Signals can also be sent to specific tasks in the \
                        current or another RTP using the taskKill() routine. \
...
Component INCLUDE_EXC_SIGNALS {
        ...
        SYNOPSIS        Enables signal generation on exception
...
Component INCLUDE_PX_SCHED_DEF_POLICIES {
        ...
        SYNOPSIS        This component provides support for POSIX SCHED_FIFO, \
...
Component INCLUDE_PX_SCHED_SPORADIC_POLICY {
        ...
        SYNOPSIS        This component provides support for POSIX SCHED_FIFO, SCHED_RR, \
...
Component INCLUDE_KERNEL_HARDENING {
        ...
        SYNOPSIS        This component applies protection to certain kernel \
                        ...
                        stacks, non-executiable heap and data sections, NULL \
                        pointer dereference detection for kernel tasks).
...
Component INCLUDE_LOGGING {
        ...
        SYNOPSIS        Provides the logLib library. This library handles \
                        message logging. It is usually used to display error \
                        messages on the system console, but such messages can \
                        also be sent to a disk file or printer. The logMsg() \
                        and logTask() routines are the basic components of the \
                        logging system. The logMsg() routine has the same \
                        calling sequence as printf(), but instead of formatting \
                        and outputting the message directly, it sends the format \
                        string and arguments to a message queue. The logTask() \
                        task waits for messages on this message queue. It \
                        formats each message according to the format string and \
                        arguments in the message, prepends the ID of the sender, \
                        and writes it on one or more file descriptors that have \
                        been specified as logging output streams (by logInit() \
...
Component INCLUDE_HASH {
        NAME            Hash library
        SYNOPSIS        This component includes the hash library. This routine \
                        ...
                        symbol tables, or other name-to-identifier routines. \
                        ...
                        user-definable routines, the hashing routine, and the \
                        hash node comparator. The hashTblCreate( ) routine \
                        creates a hash table. The hashTblInit( ) routine \
                        initializes a hash table. The hashTblDelete() routine \
                        deletes a hash table. The hashTblTerminate() routine \
                        terminates a hash table. The hashTblDestroy() routine \
                        destroys a hash table. The hashTblPut() routine puts a \
                        hash node into the specified hash table. The \
                        hashTblFind() routine finds a hash node that matches \
                        the specified key. The hashTblRemove() routine removes \
                        a hash node from a hash table. The hashTblEach() \
                        routine calls a routine for each node in a hash table. \
                        The hashFuncIterScale() routine provides an iterative \
                        scaling hashing function for strings. The hashFuncModulo() \
                        routine provides a hashing function using a remainder \
                        technique. The hashFuncMultiply() routine is a \
                        multiplicative hashing function. The hashKeyCmp() routine \
                        compares keys as 32-bit identifiers. The hashKeyStrCmp() \
                        routine compares keys based on strings they point to.
#ifndef _WRS_CONFIG_CERT_KERNEL_OBJECT
        MODULES         hashLib.o
#else
        _REQUIRES INCLUDE_KERNEL
...
Component INCLUDE_DLL {
        ...
        SYNOPSIS        This routine library supports the creation and \
                        ...
                        a reduced version of the routines offered in lstLib(1). \
...
Component INCLUDE_SLL {
        ...
        SYNOPSIS        This component includes the sslLib library. This \
                        routine library supports the creation and \
                        maintenance of a singly-linked list. The user \
                        supplies a list descriptor (type SL_LIST) that \
...
Component INCLUDE_LSTLIB {
        ...
        SYNOPSIS        This component provides the lstLib library. This routine \
                        library supports the creation and maintenance of a \
                        doubly-linked list. The user supplies a list descriptor \
...
Component INCLUDE_RNG_BUF {
        ...
        SYNOPSIS        This component provides the rngLib library. This library \
                        provides routines for creating and using ring buffers, \
                        which are first-in-first-out circular buffers. The \
                        routines simply manipulate the ring buffer data \
...
Component INCLUDE_GETPAGESIZE {
        NAME            The getpagesize() function in the kernel.
...
Parameter HOST_NAME_DEFAULT {
        ...
        SYNOPSIS        Specifies the default value for the host name field of \
                        the VxWorks boot line.
        TYPE            string
        DEFAULT         "bootHost"
}
...
Parameter TARGET_NAME_DEFAULT {
        ...
        SYNOPSIS        Specifies the default value for the target name field of \
                        the VxWorks boot line (tn).
        TYPE            string
        DEFAULT         "vxTarget"
}
...
Parameter HOST_USER_DEFAULT {
        ...
        SYNOPSIS        Specifies the default value for the host user field of \
                        the VxWorks boot line (u).
        TYPE            string
        DEFAULT         "target"
}
...
Parameter HOST_PASSWORD_DEFAULT {
        ...
        SYNOPSIS        Specifies the default value for the host password \
                        field of the VxWorks boot line.
        TYPE            string
        DEFAULT         ""
}
...
Parameter SCRIPT_DEFAULT {
        ...
        SYNOPSIS        Specifies the default value for script field of \
                        VxWorks boot line (s)
        TYPE            string
        DEFAULT         ""
}
...
Parameter OTHER_DEFAULT {
        NAME            Default other field for boot line
        SYNOPSIS        Default value for other field of VxWorks boot line (o)
        TYPE            string
        DEFAULT         ""
}
...
Component INCLUDE_ATOMIC_OPERATORS {
        ...
        SYNOPSIS        Atomic operators, such as vxAtomicAdd() and \
                        vxAtomicInc() support
...
Component INCLUDE_POOL {
        ...
        SYNOPSIS        This module contains the Memory Pool library. Pools \
                        provide a fast and efficient memory management when an \
                        aplication uses a large number of identically sized \
                        memory blocks (e.g. structures, objects) by minimizing \
                        the number of allocations from a memory partition. The \
                        use of pools also reduces possible fragmentation \
                        caused by frequent memory allocation and freeing. \
                        A pool is a dynamic set of statically sized memory \
                        blocks. All blocks in a pool are of the same size. The \
                        size of blocks are specified at pool creation and all \
                        the blocks will be aligned on the block size; the \
...
Component INCLUDE_POOL_SHOW {
        ...
        SYNOPSIS        This component provides the poolShow library. This \
                        library provides routines which display information about \
                        memory pools in the system, and detailed statistics about \
                        individual memeory pools.
        MODULES         poolShow.o
        LINK_SYMS       poolShow
...
Parameter INTERRUPT_STACK_OVERFLOW_SIZE {
        ...
        SYNOPSIS        This defines the size (in bytes) of the protected area \
                        above the interrupt stack.
        TYPE            uint
        DEFAULT         4096
}
...
Parameter DKM_TLS_SIZE {
        NAME            DKM TLS size
        SYNOPSIS        The size of extra TLS block reserved for allocating \
                        thread local variables in DKMs
        TYPE            int
        DEFAULT         0
}
...
Component INCLUDE_GETOPT {
        ...
        SYNOPSIS        The getopt function decodes arguments passed in \
                        argc/argv vectors.
        MODULES         getopt.o
        HDR_FILES       unistd.h
...
Component INCLUDE_RANDOM_NUM_GEN {
        ...
        SYNOPSIS        This component provides the randomNumGenLib library \
                        ...
                        random number. Refer to the randomNumGenLib entry in \
                        the VxWorks 7 Application COREOS API Reference for \
                        more details on the routines included in this library.
...
Component INCLUDE_SW_RANDOM_NUM_GEN {
        ...
        SYNOPSIS        This component provides the randomSWNGenLib library. \
                        ...
                        software random number. When an application requests \
                        random bytes, the content of the destination buffer is \
                        first used as random seed. Thereafter the random state \
                        buffer is hashed and the hash output is returned as \
                        random bytes. If more bytes are requested than the \
                        ...
                        of random bytes as been generated.
...
Parameter TASK_USER_EXC_STACK_SIZE {
        ...
        SYNOPSIS        This defines the size (in bytes) of the exception \
                        stack for the user tasks.
        TYPE            uint
#ifdef _WRS_CONFIG_LP64
        DEFAULT         16384
#else
...
Parameter TASK_KERNEL_EXC_STACK_SIZE {
        ...
        SYNOPSIS        This defines the size of the exception stack for \
                        kernel tasks
        TYPE            size_t
...
Parameter WIND_JOBS_MAX  {
        ...
        SYNOPSIS        WIND work queue size (number of entries). Must be a \
                        power of 2. A value below 64 may cause work queue panics.
        TYPE            UINT32
        DEFAULT         64
}
...
Parameter MAX_ISR_JOBS {
        ...
        SYNOPSIS        Maximum number of jobs posted to tExcTask from ISR \
                        level. In most applications, the default value should \
                        be sufficient to handle the jobs posted from ISR level. \
                        If the number of jobs posted exceeded MAX_ISR_JOBS, an \
                        overflow error will occur. Before increasing the value \
                        of MAX_ISR_JOBS, it is recommended to review the ISRs \
                        that are posting jobs to ensure the ISR is not improperly \
                        posting more jobs than the system can handle at any one time.
        TYPE            UINT32
        DEFAULT         16
}
...
Parameter MAX_LOG_MSGS {
        ...
        SYNOPSIS        The maximum number of messages is defined by this \
                        parameter. It is set to fifty by default.
        TYPE            uint
        DEFAULT         50
}
...
Parameter LOG_TASK_PRIORITY {
        ...
        SYNOPSIS        The LOG_TASK_PRIORITY parameter defines the priority \
                        with which the logging task tLogTask is spawned. The \
...
Parameter NUM_EARLY_SYSLOG_MSGS {
        NAME            Maximum number of early message for syslog
        SYNOPSIS        The NUM_EARLY_SYSLOG_MSGS parameter defines the number \
                        of messages that can be logged before the logging \
                        facility is initialized. The default value is zero.
        TYPE            uint
        DEFAULT         0
}
...
Parameter NUM_SIGNAL_QUEUES {
        ...
        SYNOPSIS        This parameters sets the maximum number of queued \
...
Parameter ENV_VAR_USE_HOOKS {
        ...
        SYNOPSIS        This parameter controls installHooks. If installHooks \
                        ...
                        that will optionally create and destroy private \
                        ...
                        requires a private environment, it is the application's \
                        responsibility to create and destroy the private \
                        environment, using envPrivateCreate( ) and \
                        envPrivateDestroy( ).
        TYPE            bool
        DEFAULT         TRUE
}
...
Parameter EDR_ERRLOG_SIZE {
        ...
        SYNOPSIS        This parameter sets the size of the persistent memory \
                        area reserved for error records. By default, it is set \
                        to one-half of the persistent memory available.
        DEFAULT         (pmFreeSpace (EDR_PM_ARENA) / 2)
}

Parameter EDR_RECORD_SIZE {
        NAME            Size of each ED&R record (in bytes)
        SYNOPSIS        The EDR_RECORD_SIZE parameter can be used to change \
                        the default size of error records.Note that for \
                        performance reasons, all records are necessarily the \
                        same size.
        DEFAULT         4096
}
...
Parameter PM_RESERVED_MEM {
        ...
        SYNOPSIS        The size of the persistent memory region is defined by \
                        this configuration parameter. By default, it is set to \
                        six pages of memory.
...
Parameter VX_GLOBAL_NO_STACK_FILL {
        ...
        SYNOPSIS        When set to true, this parameter will disable the \
                        stack fill.
        TYPE            BOOL
        DEFAULT         FALSE
}
...
Component INCLUDE_SPINLOCK {
        NAME            spinLock
        SYNOPSIS        spinLock libraries without debugging code
#ifdef _WRS_CONFIG_SMP
#ifndef _WRS_CONFIG_CERT_KERNEL_OBJECT
...
Folder FOLDER_KERNEL_DEBUG {
        NAME            Kernel-write
        SYNOPSIS        This folder contains components and parameters related \
                        to kernel write such as kprintf, kputs and kwrite.
...
Selection SELECT_DEBUG_KWRITE {
        ...
        SYNOPSIS        Select the component to provide the output routine for \
                        the kernel-write facility.
        COUNT           1-1
