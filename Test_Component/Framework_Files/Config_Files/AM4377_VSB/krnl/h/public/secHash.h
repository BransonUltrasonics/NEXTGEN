...
typedef struct SEC_HASH_TEMPLATE_ST
    ...
    int md_size;
    int blockSize;
    STATUS (*init)(struct SEC_HASH_CTX_ST* ctx, const struct SEC_HASH_TEMPLATE_ST* template);
...
STATUS secHash(const void* data, size_t len, unsigned char* digest, unsigned int *out_len, const SEC_HASH_TEMPLATE* template);
     ...
     * secHashCleanup(&ctx);
     */
STATUS secHashInit(SEC_HASH_CTX* ctx, const SEC_HASH_TEMPLATE* template);
