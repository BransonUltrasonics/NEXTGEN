...
void Recipe::createParamModel(QList<QObject*> *ptrModelList,QString qstrParams,QString qstrParamsLocale)
{
    ...
    for(int dwIndex=0;dwIndex<qstrParamsList.size();dwIndex++)
    {
        ...
        {
            paramValue = QString::number(round(paramValue.toDouble()));
        }
...
void Recipe::createSetupLimitsModel()
{
    ...
    for(int dwIndex=0;dwIndex<setuplist.size();dwIndex++)
    {
        ...
        {
            setupLimitsList->m_SetUpMinValue = QString::number(round(setupLimitsList->m_SetUpMinValue.toDouble()));
            setupLimitsList->m_SetUpMaxValue = QString::number(round(setupLimitsList->m_SetUpMaxValue.toDouble()));
        }
