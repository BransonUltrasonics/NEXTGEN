...
/*declare dwCurrentUnit to check unit as metric or imperial */
...
/* To check the Unit, whether it is Metric or Imperial and revert the respective string */
...
void SysConfigData:: createBransonACSystemConfigList()
{
    ...
    for(int dwIndex=0;dwIndex<bransonSysMenuList.size();dwIndex++)
    {
		...
		/*check m_ParameterName value with "SC Serial Number" OR "Stroke Length (mm)" OR Stroke Length (in) */
        else if((ptrSys->m_ParameterName ==SYS_AC_SERIAL_NUMBER)||(ptrSys->m_ParameterName ==SYS_CONFIG_STROKE_LENGTH) ||(ptrSys->m_ParameterName ==SYS_CONFIG_STROKE_LENGTH_IMP))
        {
            ptrSys->m_ParameterTextEdit= true;
            ptrSys->m_ParamTextEditName=getNameOfTextEdit(ptrSys->m_ParameterName);
            ptrSys->m_ParameterSwitch= false;
...
QString SysConfigData::getNameOfTextEdit(QString paramText)
{
    ...
    /* To update Stroke Length Value in imperial Unit on Branson screen */
    else if(paramText == SYS_CONFIG_STROKE_LENGTH_IMP)
    {
        currentValue = QString::number(Configuration::getInstance()->m_systemData.ActConfig.MaxStrokelength);
        convertedValue = Configuration::getInstance()->convertMillimeterToInches(currentValue.toDouble());
        currentName.setNum(convertedValue, 'f', 4);
    }
    else
    {
        currentName="";
...
void SysConfigData::updateComboBoxState(QString paramText,int currentState)
{
    ...
    {
        if(Configuration::getInstance()->m_systemData.LanguageIs!=(App_Language)currentState)
        {
...
{
    if(Configuration::getInstance()->m_systemData.LanguageIs!=(App_Language)a_language)
    {
