...
QString GraphTableView::convertTableValues(const QString &value, int columnIndex, const QString &unit)
{
    ...
    case GraphtableEnum::MAX_WELD_FORCE:
    case GraphtableEnum::END_HOLD_FORCE:
        /*To Fetch Max Weld Force and End Hold Force value stored in SC on the basis of system unit*/
        if (unit == "SI")
        {
            double newtonValue = value.toDouble() / 10.0;
            convertdValue = QString::number(round(newtonValue));
        }
        else
        /*To display one decimal value for imperial Unit on Analytics History table graph screen*/
        {
            double newtonValue = value.toDouble() / 10.0;
            double lbsValue = Utility::convertNtolbs(newtonValue);
            convertdValue.setNum(lbsValue,'f', 1);
        }
    ...
    case GraphtableEnum::TRIGGER_DISTANCE:
        if (unit == "SI")
           ...
           convertdValue = Utility::convertmicrometretoinches(value);
        break;
    case GraphtableEnum::TRIGGER_POINT:
    case GraphtableEnum::WELD_SONIC_POINT:
    case GraphtableEnum::HOLD_POINT:
