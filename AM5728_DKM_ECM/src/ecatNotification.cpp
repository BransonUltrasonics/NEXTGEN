...
#ifndef __ATEMDEMO_USR_CONFIG__
                                      ...
                                      )
{
    ...
    T_SLAVEJOBS                     oCurJob;
    EC_T_BYTE                       slaveId                 = 0;
    EC_T_BYTE						bySubIndexVal			= 0;
    EC_T_BOOL   					boolCompleteAccess		= 0 ;

    EC_T_DWORD 						dwSubIndexOffset		= 0 ;

    EC_T_CHAR						strTfrData[MAX_DATA_STRING_LENGTH];
    EC_T_CHAR						strTmp[MAX_BYTE_STRING_LENGTH];
    EC_T_CHAR 						ErrMsg[50] 				= {0x00};
    EC_T_CHAR 						DestMsg[100] 			= {0x00}; 

    OsMemset(strTfrData , '\0' , MAX_DATA_STRING_LENGTH);
    bVerbose = m_nVerbosePrinting;

    //LogMsg("ecatNotify Called with Notification Code = %d", dwCode);
    switch (dwCode)
    {
        /************************/
    ...
    case EC_NOTIFY_STATECHANGED: /* GEN|1 */
        {
            ...
            if (bVerbose)
            {
                LogMsg(ecatGetText(EC_TXT_MASTER_STATE_CHANGE), ecatStateToStr(pStateChangeParms->oldState), ecatStateToStr(pStateChangeParms->newState));
            }
    ...
    case EC_NOTIFY_SLAVE_STATECHANGED: /* GEN|21 */
        {
            LogMsg(ecatGetText(EC_TXT_SLAVE_STATECHANGED),
                ...
                ecatStateToStr(pNotificationDesc->desc.SlaveStateChangedDesc.newState));
        } break;
    case EC_NOTIFY_SLAVES_STATECHANGED: /* GEN|22 */
        {
            ...
            for (dwSlaveIdx = 0; dwSlaveIdx < pNotificationDesc->desc.SlavesStateChangedDesc.wCount; dwSlaveIdx++)
            {
                LogMsg(ecatGetText(EC_TXT_SLAVE_STATECHANGED),
                    ...
                    ecatStateToStr((EC_T_STATE)pNotificationDesc->desc.SlavesStateChangedDesc.SlaveStates[dwSlaveIdx].byState));
            }
        } break;
    ...
    case EC_NOTIFY_MBOXRCV:     /* MBOXRCV|0 */
            {
                ...
                case eMbxTferType_COE_SDO_DOWNLOAD:
                    ...
                    {
                        LogError(ecatGetText(EC_TXT_COE_SDO_DNLD_ERROR), pMbxTfer->eTferStatus, pMbxTfer->dwErrorCode, ecatGetText(pMbxTfer->dwErrorCode));
                        //sprintf(ErrMsg,"%s",ecatGetText(pMbxTfer->dwErrorCode));
                        //EnCodeErrorResp(DestMsg,ErrMsg,pMbxTfer->dwErrorCode);
                        SendSdoDownloadResp(pMbxTfer->MbxData.CoE.wIndex, pMbxTfer->MbxData.CoE.wStationAddress,0);
                    }
                    else
                    {
                    	//sprintf(ErrMsg,"Success");
                    	//EnCodeErrorResp(DestMsg,ErrMsg);
                    	SendSdoDownloadResp(pMbxTfer->MbxData.CoE.wIndex, pMbxTfer->MbxData.CoE.wStationAddress,1);                    
                    }
                ...
                case eMbxTferType_COE_SDO_UPLOAD:
                    if ((pMbxTfer->eTferStatus != eMbxTferStatus_TferDone) || (pMbxTfer->dwErrorCode != EC_E_NOERROR))
                    {
                        LogError(ecatGetText(EC_TXT_COE_SDO_UPLD_ERROR), pMbxTfer->eTferStatus, pMbxTfer->dwErrorCode, ecatGetText(pMbxTfer->dwErrorCode));
                    }
                    else
                    {
                    	UINT32 IndexAddr = pMbxTfer->MbxData.CoE.wIndex;
                    	UINT32 SlaveAddr = pMbxTfer->MbxData.CoE.wStationAddress;
                    	SendSdoUploadResp(IndexAddr, SlaveAddr, pMbxTfer->pbyMbxTferData, pMbxTfer->dwDataLen);
                    }
    ...
    case EC_NOTIFY_FRAME_RESPONSE_ERROR: /* ERR|10 */
        ...
        not match to the frame expected or if a expected frame was not received.
          ...
          or eRspErr_UNEXPECTED error (as the master then doesn't expect to receive this frame).
            ...
            b)	too many or too long acyclic frames are sent by the master due to a improper configuration.
            To avoid these error notifications lower the following master configuration parameters:
            ...
            */
            ...
            {
                LogError(ecatGetText(EC_TXT_FRMRESP_RETRY),
            ...
            else
            {
                ...
                LogError(ecatGetText(EC_TXT_FRMRESP_NORETRY), pszTextCause, (pErrorNotificationDesc->desc.FrameRspErrDesc.bIsCyclicFrame ? "cyclic" : "acyclic"));
            }
    ...
    case EC_NOTIFY_SLAVE_UNEXPECTED_STATE:      /* ERR|32 */
        {
            ...
            LogError(ecatGetText(EC_TXT_SLAVE_UNEXPECTED_STATE), pSlaveProp->achName, pSlaveProp->wStationAddress,
                ecatStateToStr(curState), ecatStateToStr(expState));
        } break;
    ...
    case EC_NOTIFY_SLAVES_UNEXPECTED_STATE:      /* ERR|38 */
        {
            ...
            for (dwSlaveIdx = 0; dwSlaveIdx < pErrorNotificationDesc->desc.SlavesUnexpectedStateDesc.wCount; dwSlaveIdx++)
            {
                LogError(ecatGetText(EC_TXT_SLAVES_UNEXPECTED_STATE),
                    ...
                    ecatStateToStr(pErrorNotificationDesc->desc.SlavesUnexpectedStateDesc.SlaveStates[dwSlaveIdx].curState),
                    ecatStateToStr(pErrorNotificationDesc->desc.SlavesUnexpectedStateDesc.SlaveStates[dwSlaveIdx].expState));
            }
        } break;
