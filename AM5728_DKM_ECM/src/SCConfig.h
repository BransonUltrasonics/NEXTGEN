...
/*********************/
 ...
 #define MAIN_THREAD_PRIO            ((EC_T_DWORD)2)    /* Main thread */
   ...
   #define DEMO_PARAMETERS            "-auxclk 2000 -v 2 -t 10000 -perf " \
  ...
  #endif
  #else
  	  #if RING_TOPOLOGY // ring topology, AM335x
		#define DEMO_PARAMETERS           "-auxclk 250 -v 3 -t 60000 -perf " \
                                      	  "-cpsw "                    \
										  "1 "    /* port */          \
										  "1 "    /* mode */          \
										  "0 "    /* priority */      \
	  ...
	  #else // LINE TOPOLOGY, AM335x
		#define DEMO_PARAMETERS          "-auxclk 250 -v 3 -t 600000 -perf " \
										  "-cpsw "                    \
										  "1 "    /* port */          \
										  "1 "    /* mode */          \
										  "1 "    /* priority */      \
