    ...
    ,EC_T_DWORD          dwCycErrorNotifyMask )
{
    ...
    dwRes = ecatIoControl((EC_IOCTL_LINKLAYER_MAIN | EC_LINKIOCTL_UPDATE_LINKSTATUS), &oIoCtlParms);
    if (dwRes != EC_E_NOERROR)
    {
    	//LogErr( "Error calling link-layer IOCTL (Result = %s (0x%x))", EcErrorText(dwRes), dwRes );
    	//LogErr( "Error calling link-layer IOCTL (Result = %ld)", dwRes );
    }
    oIoCtlParms.pbyInBuf      = EC_NULL;
    oIoCtlParms.dwInBufSize   = 0;
    oIoCtlParms.pbyOutBuf     = EC_NULL;
    oIoCtlParms.dwOutBufSize  = 0;
    oIoCtlParms.pdwNumOutData = EC_NULL;
    dwRes = ecatIoControl((EC_IOCTL_LINKLAYER_RED | EC_LINKIOCTL_UPDATE_LINKSTATUS), &oIoCtlParms);
    if (dwRes != EC_E_NOERROR)
    {
    	//LogErr( "Error calling link-layer IOCTL (Result = %s (0x%x))", EcErrorText(dwRes), dwRes );
    	//LogErr( "Error calling link-layer IOCTL (Result = %ld)", dwRes );
...
)
{
    ...
    case ecvendor_texas_instruments:
    {
    	switch( EPC )
    	{
    	case ecprodcode_ti_437 : 				pRet = (EC_T_CHAR*)"AM4377 / AM4379 PRU-ICSS v2.0 IDK"; break ;
    	default: 				 				pRet  = (EC_T_CHAR*)"Unknown";  break;
    	}
    }
    	break;
...
#define COE_SDO_SLAVE_MBX_SIZE  ((EC_T_DWORD)0x200)
