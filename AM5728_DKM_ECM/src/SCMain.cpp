...
volatile EC_T_BOOL bEcatDown = EC_TRUE;
extern TASK_ID Ctrl_TID;
...
EC_T_VOID vxAuxClkIsr( EC_T_INT nEvent )
{
    ...
    /* Sending event to Control task at every 250us */
    if(ERROR != Ctrl_TID)
    {
    	eventSend(Ctrl_TID,CTRL_250us);
    }
}
#endif /* VXWORKS */
...
*/
#if (defined VXWORKS)
	extern "C" int scMain(char* lpCmdLine)
...
Exit:
    ...
    exit(nRetVal);
#elif !(defined RTAI)
    /* To delete the SC_MAIN routine and exit without any error - Spawned from loadAppModule.c - VIP project */
    taskDelete(taskNameToId("SC_MAIN"));
    taskExit(0);
    return nRetVal;
#endif
}
