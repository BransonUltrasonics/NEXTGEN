...
LOCAL STATUS    am437xEpwmProbe (VXB_DEV_ID);
LOCAL STATUS    am437xEpwmAttach (VXB_DEV_ID);
STATUS			am437xPwmssClockDisable(VXB_DEV_ID, int);
STATUS 			am437xPwmssClockEnable(VXB_DEV_ID, int);
LOCAL VXB_FDT_DEV *     am437xPwmssFdtDevGet (VXB_DEV_ID,
											VXB_DEV_ID
										   );
LOCAL STATUS            am437xPwmssResFree (VXB_DEV_ID,
											VXB_DEV_ID,
											VXB_RESOURCE *
											);
LOCAL VXB_RESOURCE *    am437xPwmssResAlloc (VXB_DEV_ID,
											  VXB_DEV_ID,
											  uint32_t
											);


LOCAL VXB_DRV_METHOD am437xEpwmMethods[] = {
    { VXB_DEVMETHOD_CALL (vxbDevProbe),  (FUNCPTR)am437xEpwmProbe},
    { VXB_DEVMETHOD_CALL (vxbDevAttach), (FUNCPTR)am437xEpwmAttach},
	{ VXB_DEVMETHOD_CALL(vxbFdtDevGet),     (FUNCPTR)am437xPwmssFdtDevGet    },
	{ VXB_DEVMETHOD_CALL(vxbResourceFree),  (FUNCPTR)am437xPwmssResFree      },
	{ VXB_DEVMETHOD_CALL(vxbResourceAlloc), (FUNCPTR)am437xPwmssResAlloc     },	
  	VXB_DEVMETHOD_END
};

VXB_DRV vxbOfAm437xEpwmDrv =
    {
    { NULL } ,
	"ti,am4377-pwmss",                 /* Name */
    "TI AM437x EPWM Driver",         /* Description */
	VXB_BUSID_FDT,                      /* Class */
    0,                                  /* Flags */
    0,                                  /* Reference count */
	(VXB_DRV_METHOD*)&am437xEpwmMethods  /* Method table */
    };

VXB_DRV_DEF (vxbOfAm437xEpwmDrv)

LOCAL const VXB_FDT_DEV_MATCH_ENTRY am437xEpwmMatch[] =
    {
        {
...
/*********************************************************************************
 * 
 * am437xEpwmProbe : This function finds and compare the compatible field from 
 * the dts file.
 * 
 * if found then success , else fail.
 */

LOCAL STATUS am437xEpwmProbe
    (
    struct vxbDev * pDev
    )
    {
	int ret = 0;
	ret = vxbFdtDevMatch (pDev, am437xEpwmMatch, NULL);
	return ret;
    }
...
/*********************************************************************************
*
* am437xPwmssFdtDevGet - get the FDT bus device information
*
...
* am437xPwmssFdtDevGet (pDev); in the child node.
*
...
LOCAL VXB_FDT_DEV * am437xPwmssFdtDevGet
    (
    VXB_DEV_ID      pDev,
    VXB_DEV_ID      pChild
    )
...
/*******************************************************************************
*
* am437xPwmssResFree - free resource allocated by calling vxbResourceAlloc
*
...
LOCAL STATUS am437xPwmssResFree
    (
    VXB_DEV_ID     pDev,
    VXB_DEV_ID     pChild,
    VXB_RESOURCE * pRes
...
/*******************************************************************************
*
* am437xPwmssResAlloc - allocate resource from device resource list
*
...
* ERRNO: N/A
*/
LOCAL VXB_RESOURCE * am437xPwmssResAlloc
    (
    VXB_DEV_ID      pDev,
    VXB_DEV_ID      pChild,
    uint32_t        id
...
LOCAL STATUS am437xEpwmAttach
	...
	pwmss->ehrpwmClkConfigDisable	= am437xPwmssClockDisable;
	pwmss->ehrpwmClkConfigEnable 	= am437xPwmssClockEnable;
	pwmssRegister(pDev,pwmss,pFdtDev->name);
	
	am437xPwmssClockDisable(pDev,EHRPWM);
...
/*******************************************************************************
*
* am437xPwmssClockEnable - This functions enables clock for PWMSS subsystem
*
* RETURNS: OK, or ERROR.
*
* ERRNO: N/A.
...
/******************************************************************************
*
* am437xPwmssClockDisable - This functions disables clock for PWMSS subsystem
*
* RETURNS: OK, or ERROR.
*
* ERRNO: N/A.
...
STATUS am437xPwmssClockDisable
	(
	VXB_DEV_ID pDev,
	int module
	)
