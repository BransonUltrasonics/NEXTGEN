/* httpMimePersist.c - parse functions for MIME header */
...
-------------------- 
01b,02feb10,h_y   support for LP64 
...
/*******************************************************************************
*
* httpMimePersist - parse MIME "Persist" header field
*
* This routine parses the MIME "Persist" header field. This routine is invoked
* by the MIME parser to extract the "Persist:" header from the clients HTTP
* request. This tag is an alternative to "connection: keep-alive" used by
* pre HTTP 1.1 clients to establish an persistent connection.
...
HTTP_STATUS httpMimePersist
    ...
    {
    logInfo ("MIME PERSIST:  detected");

    if (reqId->sProtocol < 1)                       /* If Protocol <= HTTP/1.0 (magic position!) */
    {
    reqId->fPersistent = HTTP_TRUE;
    logInfoMsg ("Connection set to persistent mode :%d",reqId->fPersistent);
    }
