/* vxbTiAm335xSpi.c - Ti AM335x SPI Controller Driver */
...
INCLUDE FILES: vxbSpiLib.h vxbTiAm335xSpi.h
...
LOCAL UINT32 spiDbgMask = SPI_DBG_RW;
...
LOCAL STATUS am335xSpiAttach
    ...
    /* Register Instance and Function Routines with Library */
    pMcSpiFunc = (VXB_MC_SPI_FUNC *) vxbMemAlloc (sizeof (VXB_MC_SPI_FUNC));
    pMcSpiFunc->mcspiTrans = am335xSpiTransfer;
...
*/
 ...
 LOCAL STATUS am335xSpiChanCfg
    ...
    VXB_FDT_DEV *           pFdtDev;
    // char * spiDev1 = "mcspi1"; // SPI driver name for Digital IO
    char    * port0  = "mcspi0";
    char    * port1  = "mcspi1";
    char    * port2  = "mcspi2";
    char    * port3  = "mcspi3";
    char    * port4  = "mcspi4";   
    int     strLen; 
    UINT32  max_channel = SPI_MAX_CS_NUM;   //default 4 slaves for port0, port1 and port2

    pFdtDev = vxbFdtDevGet(pDev);

    strLen = strlen(pFdtDev->name);
	
    if(strncmp(pFdtDev->name, port3, strLen) == 0) 
	{
    	max_channel = SPI_MAX_CS_NUM_2;
//		kprintf("name %s port3 %s\n", pFdtDev->name, port3);
	}
    else if(strncmp(pFdtDev->name, port4, strLen) == 0)
	{
    	max_channel = SPI_MAX_CS_NUM_2;
//		kprintf("name %s port4 %s\n", pFdtDev->name, port4);
	}  
...
//    if (channel >= SPI_MAX_CS_NUM)
    ...
    if (divideRatio > MAX_DIVIDERATIO_2)
		...
		//Frank disable other slaves
		if(0 == channel)
		{
			CSR_CLRBIT_4 (pDev, MCSPI_CHCTRL(1), ~MCSPI_CH0CTRL_EN);
			CSR_CLRBIT_4 (pDev, MCSPI_CHCTRL(2), ~MCSPI_CH0CTRL_EN);
			CSR_CLRBIT_4 (pDev, MCSPI_CHCTRL(3), ~MCSPI_CH0CTRL_EN);
			CSR_SETBIT_4 (pDev, MCSPI_CHCONF(1), (1 << MCSPI_CH0CONF_EPOL_SHIFT));            
			CSR_SETBIT_4 (pDev, MCSPI_CHCONF(2), (1 << MCSPI_CH0CONF_EPOL_SHIFT));            
			CSR_SETBIT_4 (pDev, MCSPI_CHCONF(3), (1 << MCSPI_CH0CONF_EPOL_SHIFT));            
		}
		else if(1 == channel)
		{
			CSR_CLRBIT_4 (pDev, MCSPI_CHCTRL(0), ~MCSPI_CH0CTRL_EN);
			CSR_CLRBIT_4 (pDev, MCSPI_CHCTRL(2), ~MCSPI_CH0CTRL_EN);
			CSR_CLRBIT_4 (pDev, MCSPI_CHCTRL(3), ~MCSPI_CH0CTRL_EN);
			CSR_SETBIT_4 (pDev, MCSPI_CHCONF(0), (1 << MCSPI_CH0CONF_EPOL_SHIFT));            
			CSR_SETBIT_4 (pDev, MCSPI_CHCONF(2), (1 << MCSPI_CH0CONF_EPOL_SHIFT));            
			CSR_SETBIT_4 (pDev, MCSPI_CHCONF(3), (1 << MCSPI_CH0CONF_EPOL_SHIFT));            
		}
		else if(2 == channel)
    ...
    if ((devInfo->mode & SPI_CSPOL_HIGH) != 0)
        {
        spiConf &= ~(1 << MCSPI_CH0CONF_EPOL_SHIFT);  /* high active */
        }
    else
        {
    ...
    if(strncmp(pFdtDev->name, port1, strLen) == 0)
    {
       spiConf |= MCSPI_CH0CONF_DPE1;
       spiConf |= MCSPI_CH0CONF_IS;
    }
       
    /* 1.5 clock cycles */

    //Frank 0.5 clock cycle spiConf |= MCSPI_CH0CONF_TCS;
...
#if 0
    value = CSR_READ_4 (pDev, MCSPI_CHCTRL (channel));
    SPI_DBG (SPI_DBG_RTN, "%s:Channel Control Register 0x%08x\n",__FUNCTION__,value);
    
    value = CSR_READ_4 (pDev, MCSPI_CHCONF (channel));
    SPI_DBG (SPI_DBG_RTN, "%s:Channel Configuration 0x%08x\n",__FUNCTION__,value);
#endif
    SPI_DBG (SPI_DBG_RTN,
...
LOCAL STATUS am335xSpiTrans
...
#if 0
    value = CSR_READ_4 (pDev, MCSPI_CHCTRL (channel));
    SPI_DBG (SPI_DBG_RTN, "%s:Channel Control 0x%08x\n",__FUNCTION__,value);
    
    value = CSR_READ_4 (pDev, MCSPI_CHCONF (channel));
    SPI_DBG (SPI_DBG_RTN, "%s:Channel Conf 0x%08x\n",__FUNCTION__,value);
#endif
    ...
    for (i = 0; i < SPI_FIFO_LEN && i < len; i += alignSize)
        ...
        if (pDrvCtrl->txLeft > 0)
            ...
            SPI_DBG (SPI_DBG_RW, "Tx Data: 0x%x\n", data);
        ...
        if (IS_RX_INT (channel, intSts))
            ...
            for (i = 0; i < SPI_FIFO_LEN && i < len; i += alignSize)
                ...
                if (skipRxCnt > 0)
                    ...
                    SPI_DBG (SPI_DBG_RW, "Rx Data: 0x%x\n", data);
...
#if 0
                value = CSR_READ_4 (pDev, MCSPI_TX0);
                SPI_DBG (SPI_DBG_RTN, "Channel 0 Transmit Register:%x \n",value);
                
                value = CSR_READ_4 (pDev, MCSPI_RX0);
                SPI_DBG (SPI_DBG_RTN, "Channel 0 Receive Register:%x \n",value);
//#if 0
                value = CSR_READ_4 (pDev, MCSPI_CHSTAT (channel));
                SPI_DBG (SPI_DBG_RTN,"Channel %d Status Values:%x \n",channel,value);
                
                value = CSR_READ_4 (pDev, MCSPI_RX (channel));
                SPI_DBG (SPI_DBG_RTN, "Received Value from Channel :%x \n",value);
                
                value = CSR_READ_4 (pDev, MCSPI_SYST);
                SPI_DBG (SPI_DBG_RTN, "SYST Register:%x \n",value);
                
                value = CSR_READ_4 (pDev, MCSPI_CH0CONF);
                SPI_DBG (SPI_DBG_RTN, "Channel 0 Configuration Register:%x \n",value);
                
                value = CSR_READ_4 (pDev, MCSPI_CH0STAT);
                SPI_DBG (SPI_DBG_RTN, "Channel 0 STAT Register:%x \n",value);
                
                value = CSR_READ_4 (pDev, MCSPI_CH0CTRL);
                SPI_DBG (SPI_DBG_RTN, "Channel 0 Control Register:%x \n",value);
#endif                           
                }
