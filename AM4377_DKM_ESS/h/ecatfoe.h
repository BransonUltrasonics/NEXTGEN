...
typedef struct MBX_STRUCT_PACKED_START
{
    ...
    ESS_UINT8       Data[BL_PAGE_SIZE];
#else
    ESS_UINT16      Data[BL_PAGE_SIZE>>1];
#endif
...
typedef struct  MBX_STRUCT_PACKED_START
{
    ESS_UINT16        OpCode; /**< \brief OpCode
                             *
                             * 1 : RRQ<br>
                             * 2 : WRQ<br>
                             * 3 : DATA<br>
    ...
    {
        ESS_UINT32        Password; /**< \brief Password (used in Read request and Write request). 0 if unknown*/
        ESS_UINT32        PacketNo; /**< \brief Packet number (used in DATA and ACK datagram)*/
        ESS_UINT32        ErrorCode; /**< \brief Error code (used in ERR datagram)*/
        struct MBX_STRUCT_PACKED_START
        {
            ESS_UINT16    Done; /**< \brief Done indication (used in BUSY datagram)*/
            ESS_UINT16    Entire;  /**< \brief Entire indication (used in BUSY datagram)*/
        }MBX_STRUCT_PACKED_END
...
typedef struct MBX_STRUCT_PACKED_START
{
    ...
    ESS_UINT16              Data[((MAX_MBX_DATA_SIZE)-(FOE_HEADER_SIZE)) >> 1]; /**< \brief FoE Data buffer*/
}MBX_STRUCT_PACKED_END
TFOEMBX;
...
------
-----------------------------------------------------------------------------------------*/
PROTO    ESS_UINT32                                 u32PacketNo; /**< \brief Stores the actual packet number to be expected*/
PROTO    ESS_UINT32                                 u32FileOffset;  /**< \brief Stores the next file offset to be sent */
PROTO    ESS_UINT32                                 u32LastFileOffset; /**< \brief Stores the actual sent file offset */
PROTO    ESS_UINT16                                 u16FileAccessState; /**< \brief Stores the actual state of the file transmission sequence */
...
PROTO    ESS_UINT8 FOE_ServiceInd(TFOEMBX MBXMEM * pFoeInd);
