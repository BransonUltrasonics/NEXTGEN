...
V5.11 MBX1: "return value of ""MBX_StartMailboxHandler()"" changed to ESS_UINT16"<br>
...
#define     EMCY_SERVICE                        ((ESS_UINT8) 0x0001) /**< \brief Emergency service*/
#define     COE_SERVICE                         ((ESS_UINT8) 0x0002) /**< \brief CoE service*/
#define     SOE_SERVICE                         ((ESS_UINT8) 0x0004) /**< \brief SoE service*/
#define     EOE_SERVICE                         ((ESS_UINT8) 0x0008) /**< \brief EoE service*/
#define     AOE_SERVICE                         ((ESS_UINT8) 0x0010) /**< \brief AoE service*/
#define     VOE_SERVICE                         ((ESS_UINT8) 0x0020) /**< \brief VoE service*/
#define     FOE_SERVICE                         ((ESS_UINT8) 0x0040) /**< \brief FoE service*/
#define     FRAGMENTS_FOLLOW                    ((ESS_UINT8) 0x0080) /**< \brief Fragments follow service*/
...
{
    ESS_UINT16                          Length; /**< \brief Length*/
    ESS_UINT16                          Address; /**< \brief Address*/

#if MBX_16BIT_ACCESS
    ESS_UINT16                          Flags[1]; /**< \brief Flags*/
...
typedef struct MBX_STRUCT_PACKED_START
{
    ...
    ESS_UINT16                          Data[(MAX_MBX_DATA_SIZE >> 1)]; /**< \brief Mailbox data*/
}MBX_STRUCT_PACKED_END
TMBX;
...
typedef struct
{
    ...
    ESS_UINT16    maxQueueSize; /**< \brief Max queue size*/
...
PROTO ESS_BOOL                    bReceiveMbxIsLocked; /**< \brief Receive mailbox is locked (MBoxOut, default SM0)*/
PROTO ESS_BOOL                    bSendMbxIsFull; /**< \brief Send mailbox is full (MBoxIn, default SM1)*/
PROTO ESS_BOOL                    bMbxRunning; /**< \brief Mailbox running (At least PreOP state)*/
PROTO ESS_BOOL                    bMbxRepeatToggle; /**< \brief Mailbox repeat toggle*/
PROTO ESS_UINT16                  u16SendMbxSize; /**< \brief Send mailbox size (default SM1)*/
PROTO ESS_UINT16                  u16ReceiveMbxSize; /**< \brief Receive mailbox size (default SM0)*/
PROTO ESS_UINT16                  u16EscAddrReceiveMbx; /**< \brief Receive mailbox address (default SM0)*/
PROTO ESS_UINT16                  u16EscAddrSendMbx; /**< \brief Send mailbox address (default SM1)*/
PROTO ESS_UINT8                   u8MbxWriteCounter; /**< \brief Write mailbox counter*/
PROTO ESS_UINT8                   u8MbxReadCounter; /**< \brief Read mailbox counter*/
PROTO TMBX MBXMEM             asMbx[2]; /**< \brief Mailbox buffer (for repeat service)*/
PROTO ESS_UINT8                   u8MailboxSendReqStored; /**< \brief Mailbox send request stored*/
...
PROTO ESS_BOOL                    bPendingMbxResponse;   /**indicates if a Mbox is pending (in case that no mailbox queue is supported only one mailbox request after the other shall be handled)*/
...
PROTO   ESS_UINT16    MBX_StartMailboxHandler(void);
...
PROTO   ESS_UINT8    MBX_MailboxSendReq(TMBX MBXMEM * pMbx, ESS_UINT8 flags);
PROTO   void     MBX_CheckAndCopyMailbox(void);
PROTO   ESS_UINT8    MBX_CopyToSendMailbox(TMBX MBXMEM *pMbx);
