...
V4.01 ECAT 2: APPL_StopOutputHandler returns an ESS_UINT16<br>
...
V4.00 ECAT 7: The return values for the AL-StatusCode were changed to ESS_UINT16
*/
...
------
-----------------------------------------------------------------------------------------*/
ESS_UINT16    u16ALEventMask;                      // Value which will be written to the 0x204 register (AL event mask) during the state transition PreOP to SafeOP

/*Dummy variable to trigger read or writes events in the ESC*/
#if ESC_32BIT_ACCESS
    VARVOLATILE ESS_UINT32    u32dummy;
#elif ESC_16BIT_ACCESS
    VARVOLATILE ESS_UINT16    u16dummy;
#else
    VARVOLATILE ESS_UINT8 u8dummy;
#endif
 ...
 */
ESS_BOOL bSyncSetByUser = FALSE;
#endif

#if ESC_32BIT_ACCESS
        VARVOLATILE ESS_UINT32 SMActivate = 0;
#elif ESC_16BIT_ACCESS
        VARVOLATILE ESS_UINT16 SMActivate = 0;
#else
        VARVOLATILE ESS_UINT8 SMActivate = 0;
#endif
...
*////////////////////////////////////////////////////////////////////////////////////////
void SetALEventMask(ESS_UINT16 intMask)
{
    #if ESC_32BIT_ACCESS
    ESS_UINT32 u32Mask = 0;
    HW_EscReadDWord(u32Mask, ESC_AL_EVENTMASK_OFFSET);
...
TSYNCMAN ESCMEM * GetSyncMan( ESS_UINT8 channel )
{
...
*////////////////////////////////////////////////////////////////////////////////////////
void DisableSyncManChannel(ESS_UINT8 channel)
{
    ESS_UINT16 Offset;
#if ESC_32BIT_ACCESS
    //The registers from 0x804 to 0x806 are only readable from PDI => writing 0 for all registers is valid
    VARVOLATILE ESS_UINT32 smStatus = SM_SETTING_PDI_DISABLE;
    ...
    VARVOLATILE ESS_UINT16 smStatus = SM_SETTING_PDI_DISABLE;
...
#else
    VARVOLATILE ESS_UINT8 smStatus = SM_SETTING_PDI_DISABLE;
...
*////////////////////////////////////////////////////////////////////////////////////////
void EnableSyncManChannel(ESS_UINT8 channel)
{
    ESS_UINT16 Offset;
#if ESC_32BIT_ACCESS
    //The registers from 0x804 to 0x806 are only readable from PDI => writing 0 for all registers is valid
    VARVOLATILE ESS_UINT32 smStatus = 0x00000000;
    Offset = (ESC_SYNCMAN_CONTROL_OFFSET + (SIZEOF_SM_REGISTER*channel));
...
#elif ESC_16BIT_ACCESS
    //The register 0x806 is only readable from PDI => writing 0 is valid
    VARVOLATILE ESS_UINT16 smStatus = 0x0000;
    Offset = (ESC_SYNCMAN_ACTIVE_OFFSET + (SIZEOF_SM_REGISTER*channel));
...
#else
    VARVOLATILE ESS_UINT8 smStatus = 0x00;
    Offset = (ESC_SM_PDICONTROL_OFFSET + (SIZEOF_SM_REGISTER*channel));
...
*////////////////////////////////////////////////////////////////////////////////////////
void Diag_DumpInvalidSMSetting(ESS_UINT8 channel, ESS_UINT8 faultyCode,TSYNCMAN ESCMEM *pSyncMan)
{
    ESS_UINT8 NumParam = 2;
    TDIAGMSGPARAMINFO aParam[2];
    ESS_UINT16 TextID = 0;
    ESS_UINT16 FalseValue = 0;
    ESS_UINT8 FailedSmChannel = channel;
    ...
    if(faultyCode > 0)
    {
        switch(faultyCode)
        {
...
#if ESC_32BIT_ACCESS
                FalseValue = (ESS_UINT16)((pSyncMan->AddressLength & SM_LENGTH_MASK) >> SM_LENGTH_SHIFT);
#else
            ...
            case SYNCMANCHSIZE    :
#if ESC_32BIT_ACCESS
                FalseValue = (ESS_UINT16)(pSyncMan->AddressLength & SM_ADDRESS_MASK);
#else
...
#if ESC_32BIT_ACCESS
                FalseValue = (ESS_UINT16) (pSyncMan->Settings[SM_SETTING_CONTROL_OFFSET] & SM_SETTING_CONTROL_MASK);
#else
...
/* ECATCHANGE_START(V5.11) DIAG4*/
        aParam[0].ParamFlags = (DIAG_MSG_PARAM_TYPE_DATA << DIAG_MSG_PARAM_TYPE_OFFSET) | (ESS_UINT16)DEFTYPE_UNSIGNED8;
        aParam[0].pParam = (MEM_ADDR *)&FailedSmChannel;

        aParam[1].ParamFlags = (DIAG_MSG_PARAM_TYPE_DATA << DIAG_MSG_PARAM_TYPE_OFFSET) | (ESS_UINT16)DEFTYPE_UNSIGNED16;
...
void    SendSmFailedEmergency(ESS_UINT8 channel, ESS_UINT8 faultyCode)
{
...
#if ESC_32BIT_ACCESS
                    pEmcy->Emcy.RegData[EMCY_OFFS_DIAGDATA+1] = SWAPWORD(((ESS_UINT16) (SM_SETTING_ENABLE_VALUE >> SM_SETTING_ENABLE_SHIFT)));
#else
...
ESS_UINT8    CheckSmSettings(ESS_UINT8 maxChannel)
{
    ESS_UINT8 i;
    ESS_UINT8 result = 0;
    TSYNCMAN ESCMEM *pSyncMan;
    ESS_UINT16 SMLength = 0;
    ESS_UINT16 SMAddress = 0;
...
#if ESC_32BIT_ACCESS
    SMLength = (ESS_UINT16)((pSyncMan->AddressLength & SM_LENGTH_MASK) >> SM_LENGTH_SHIFT);
    SMAddress = (ESS_UINT16)(pSyncMan->AddressLength & SM_ADDRESS_MASK);
...
/* ECATCHANGE_END(V5.11) HW2*/
    ...
    if ( (pSyncMan->Settings[SM_SETTING_ACTIVATE_OFFSET] & SM_SETTING_ENABLE_VALUE) != 0 && SMLength == 0 )
        ...
        else if (pSyncMan->Settings[SM_SETTING_ACTIVATE_OFFSET] & SM_SETTING_ENABLE_VALUE)
        {
            ...
            if ( SMLength == nPdOutputSize && nPdOutputSize != 0 && SMLength <= ((ESS_UINT16)MAX_PD_OUTPUT_SIZE))
...
ESS_UINT16 StartInputHandler(void)
{
    TSYNCMAN ESCMEM * pSyncMan;
#if ESC_32BIT_ACCESS
     ESS_UINT32        dcControl;
#elif ESC_16BIT_ACCESS
    ESS_UINT16        dcControl;
#else
    ESS_UINT8        dcControl;
#endif
    ESS_UINT16     wdiv = 0;
...
#endif
#if DC_SUPPORTED
    ESS_UINT32     cycleTimeSync0 = 0; /* Sync0 cycle time */
    ESS_UINT32     cycleTimeSync1 = 0; /* Delay between the Sync0 and Sycn1 signal. A new Sync1 cycle starts on the next Sync0 signal after Sync1 signal.*/
    ESS_BOOL bSubordinatedCycles = FALSE;
#endif // DC_SUPPORTED

#if MAX_PD_INPUT_SIZE > 0
    ESS_UINT16    nPdInputBuffer = 3;
#endif
#if MAX_PD_OUTPUT_SIZE > 0
    ESS_UINT16    nPdOutputBuffer = 3;
#endif

    ESS_UINT16 SyncType0x1C32 = 0; /* Helper variable for sync type for SM2 (required if no CoE is supported or no output process data available)*/
    ESS_UINT16 SyncType0x1C33 = 0; /* Helper variable for sync type for SM3 (required if no CoE is supported or no input process data available)*/

#if COE_SUPPORTED
    ESS_UINT16 u16MinSuppSyncType = 0xFFFF;  /* Minimum supported Sync Types */
...
#if ESC_32BIT_ACCESS
    nEscAddrOutputData = (ESS_UINT16) (pSyncMan->AddressLength & SM_ADDRESS_MASK);
#else
...
#if ESC_32BIT_ACCESS
    nEscAddrInputData = (ESS_UINT16) (pSyncMan->AddressLength & SM_ADDRESS_MASK);
#else
...
/*ECATCHANGE_START(V5.11) ESM3*/
                if((SyncType0x1C32 == (ESS_UINT16)SYNCTYPE_DCSYNC1)
                    ||(SyncType0x1C33 == (ESS_UINT16)SYNCTYPE_DCSYNC1))
...
/*ECATCHANGE_START(V5.11) ESM3*/
                if((SyncType0x1C32 == (ESS_UINT16)SYNCTYPE_DCSYNC0)
                    ||(SyncType0x1C33 == (ESS_UINT16)SYNCTYPE_DCSYNC0))
/*ECATCHANGE_END(V5.11) ESM3*/
                {
#if DIAGNOSIS_SUPPORTED
                    Diag_CreateNewMessage(DIAG_CODE_APPL_SYNC,DIAG_MSG_TYPE_ERROR,DIAG_TEXT_ID_INVALID_SYNC_TYPE,0,NULL);
            ...
            if (nPdOutputSize > 0)
            {
                if ((SyncType0x1C32 == (ESS_UINT16)SYNCTYPE_SM_SYNCHRON) || ((nPdInputSize > 0) && (SyncType0x1C33 == (ESS_UINT16)SYNCTYPE_SM2_SYNCHRON)))
                {
#if DIAGNOSIS_SUPPORTED
                    Diag_CreateNewMessage(DIAG_CODE_APPL_SYNC, DIAG_MSG_TYPE_ERROR, DIAG_TEXT_ID_INVALID_SYNC_TYPE, 0, NULL);
#endif
            ...
            {
                if (SyncType0x1C33 == (ESS_UINT16)SYNCTYPE_SM_SYNCHRON)
                {
#if DIAGNOSIS_SUPPORTED
                    Diag_CreateNewMessage(DIAG_CODE_APPL_SYNC, DIAG_MSG_TYPE_ERROR, DIAG_TEXT_ID_INVALID_SYNC_TYPE, 0, NULL);
#endif
    ...
    else
    {
...
#endif //#else #if !DC_SUPPORTED
        {
            ...
            /* Forcing the Sync Type to free run mode for the time being. This will be more accessed when we use mailboxes */
          //  SyncType0x1C32 = SYNCTYPE_FREERUN ;		// disabling Polling mode
           // SyncType0x1C33 = SYNCTYPE_FREERUN ;
    ...
    {
#if MAX_PD_OUTPUT_SIZE > 0
        sSyncManOutPar.u32Sync0CycleTime = (ESS_UINT32)cycleTimeSync0;
#endif

#if MAX_PD_INPUT_SIZE > 0
/*ECATCHANGE_START(V5.11) ECAT4*/
        sSyncManInPar.u32Sync0CycleTime = (ESS_UINT32)cycleTimeSync0;
/*ECATCHANGE_END(V5.11) ECAT4*/
#endif
    }
...
#if MAX_PD_INPUT_SIZE > 0
/*ECATCHANGE_START(V5.11) ECAT4*/
        sSyncManInPar.u32Sync0CycleTime = (ESS_UINT16)cycleTimeSync0;
/*ECATCHANGE_END(V5.11) ECAT4*/
#endif    
    }
#endif
...
/*ECATCHANGE_END(V5.11) ECAT4*/
    ...
    if(bSubordinatedCycles == TRUE)
    {
        ...
        {
            u16SmSync0Value = (ESS_UINT16)(cycleTimeSync1 / cycleTimeSync0);
        ...
        LatchInputSync0Value = (ESS_UINT16) (cycleTimeSync1 / cycleTimeSync0);
...
#if ESC_32BIT_ACCESS
    {
    ESS_UINT32 tmpValue = 0;
    HW_EscReadDWord(tmpValue, ESC_PD_WD_TIME);

    wd = (ESS_UINT16)(SWAPDWORD(tmpValue & 0x0000FFFF));
    }
#else
...
/*ECATCHANGE_END(V5.11) ECAT4*/
    ...
    wdiv = (ESS_UINT16)(tmpValue & 0x0000FFFF);
    }
#else
...
/*ECATCHANGE_END(V5.11) ECAT4*/
    ...
    if ( (dcControl & ESC_DC_SYNC0_ACTIVE_MASK) != 0 )
    {
        ...
        {
            ESS_UINT32 Sync0Cycle = cycleTimeSync0/100000;
            if(Sync0Cycle < 5)
            {
            ...
            {
                Sync0WdValue = (ESS_UINT16)(Sync0Cycle*2)/10;
            }
        }
        ...
        else
        {
                ...
                ESS_UINT32 Sync1Cycle = cycleTimeSync1/100000;
                if(Sync1Cycle < 5)
                {
                    /*Sync0 cycle less than 500us*/
                    Sync1WdValue = 1;
    ...
    {
                    Sync1WdValue = (ESS_UINT16)(Sync1Cycle*2)/10;
                }
...
ESS_UINT16 StartOutputHandler(void)
{
/*ECATCHANGE_START(V5.11) ESM6*/
    ...
    ESS_UINT16 result = NOERROR_INWORK;
...
#if AL_EVENT_ENABLED
    {
        ESS_UINT16 ResetMask = SYNC0_EVENT | SYNC1_EVENT;
#if MAX_PD_OUTPUT_SIZE > 0
...
*////////////////////////////////////////////////////////////////////////////////////////
void SetALStatus(ESS_UINT8 alStatus, ESS_UINT16 alStatusCode)
{
    ESS_UINT16 Value = alStatusCode;
#if ESC_32BIT_ACCESS
    ESS_UINT32 tmpValue = 0;
#endif
    ...
    {
#if ESC_32BIT_ACCESS
        tmpValue = SWAPDWORD((ESS_UINT32) Value);
    ...
    {
    ESS_UINT32 TmpVar = 0;
    TmpVar = SWAPDWORD((((ESS_UINT32)u8EcatRunLed) | (((ESS_UINT32)u8EcatErrorLed)<<8)));
    ...
    {
    ESS_UINT16 TmpVar = 0;
    TmpVar = SWAPWORD((((ESS_UINT16)u8EcatRunLed) | (((ESS_UINT16)u8EcatErrorLed)<<8)));
...
void AL_ControlInd(ESS_UINT8 alControl, ESS_UINT16 alStatusCode)
{
    ESS_UINT16        result = 0;
    ESS_UINT8            bErrAck = 0;
    ESS_UINT8         stateTrans;
...
void AL_ControlRes(void)
{
    if(bEcatWaitForAlControlRes)
    {
        ESS_UINT16 result = 0;
        ESS_UINT8 Status = 0;
        ESS_UINT16 StatusCode = 0;

        if(EsmTimeoutCounter == 0)
        {
            Status =  (ESS_UINT8)(nEcatStateTrans >> 4);
                ...
                case INIT_2_BOOT:
                    if(bApplEsmPending)
                    {
                        ...
                        if(result == 0)
                        {
                            ...
                            bMbxRunning = TRUE;
                            Status =  (ESS_UINT8)(nEcatStateTrans & STATE_MASK);
                        }
                        else
                        {
...
#else
            if(u16SmEventMissedCounter < (ESS_UINT16)MAX_SM_EVENT_MISSED)
...
void ECAT_StateChange(ESS_UINT8 alStatus, ESS_UINT16 alStatusCode)
{
    ESS_UINT8 Status = alStatus;
...
void ECAT_Init(void)
{
    ...
    TmpVar = SWAPDWORD(TmpVar);
    nMaxSyncMan = (ESS_UINT8)((TmpVar & ESC_SM_CHANNELS_MASK) >> ESC_SM_CHANNELS_SHIFT);

    //get max address (register + DPRAM size in Byte (in the register it is stored in KB))
    /* ECATCHANGE_START(V5.11) ESC1*/
    nMaxEscAddress = (ESS_UINT16)(((TmpVar & ESC_DPRAM_SIZE_MASK) >> ESC_DPRAM_SIZE_SHIFT) << 10) + 0xFFF;
    ...
    //get max address (register + DPRAM size in Byte (in the register it is stored in KB))
    /* ECATCHANGE_START(V5.11) ESC1*/
    nMaxEscAddress = (ESS_UINT16) ((TmpVar & ESC_DPRAM_SIZE_MASK) << 10) + 0xFFF;
    /* ECATCHANGE_END(V5.11) ESC1*/
    }
#else
...
void ECAT_Main(void)
{
    ESS_UINT16 ALEventReg;
    ESS_UINT16 EscAlControl = 0x0000;
#if MAILBOX_SUPPORTED
#if ESC_32BIT_ACCESS
     ESS_UINT32 sm1Activate = SM_SETTING_ENABLE_VALUE;
#elif ESC_16BIT_ACCESS
    ESS_UINT16 sm1Activate = SM_SETTING_ENABLE_VALUE;
#else
    ESS_UINT8 sm1Activate = SM_SETTING_ENABLE_VALUE;
#endif
#endif
    ...
    if ((ALEventReg & AL_CONTROL_EVENT) && !bEcatWaitForAlControlRes)
    {
        ...
        EscAlControl = (ESS_UINT16) SWAPDWORD(tmpVal);
#elif ESC_16BIT_ACCESS
    ...
    bExplicitDevIdRequested = ((EscAlControl & (ESS_UINT16)STATE_DEVID)>>5);
#endif
        ...
        AL_ControlInd((ESS_UINT8)EscAlControl, 0); /* in AL_ControlInd the state transition will be checked and done */
