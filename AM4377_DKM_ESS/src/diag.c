...
------
---------------------------------------------------------------------------------------*/
ESS_UINT8 u8LastDiagMsgRead = 0; /*Local Index to store the last read message*/
/*---------------------------------------------------------------------------------------
------
------    Functions
------
...
*////////////////////////////////////////////////////////////////////////////////////////
ESS_UINT8 ClearMessageHistory(void)
{
    ESS_UINT8 result = 0;
...
/*ECATCHANGE_END(V5.11) DIAG2*/
    ...
    else
    {
        ...
        ESS_UINT8 MessagesLeft = 0; //indicates how many messages are left in the message history queue
...
/*ECATCHANGE_END(V5.11) DIAG2*/
    ...
    sDiagHistory.Flags &= ~((ESS_UINT16)DIAG_OVERWRITE_DISCARD);
...
*////////////////////////////////////////////////////////////////////////////////////////
ESS_UINT8 Write0x10F3( ESS_UINT16 index, ESS_UINT8 subindex, ESS_UINT32 dataSize, ESS_UINT16 MBXMEM * pData, ESS_UINT8 bCompleteAccess )
{
    ESS_UINT8 result = 0;
    ...
    if(subindex == 3)
    {
        ...
        ESS_UINT8 NewValueSi3 = ((ESS_UINT8 MBXMEM *)pData)[0];
        ...
        else if (NewValueSi3 >= 6 && NewValueSi3 <= (ESS_UINT8)sDiagHistory.u16SubIndex0)
        {
    ...
    else if (subindex == 5)
    {
        ...
        */
        ESS_UINT16 ReadOnlyMask = (ESS_UINT16)~(DIAG_DISABLE_INFO_MSG | DIAG_DISABLE_WARNING_MSG | DIAG_DISABLE_ERROR_MSG);
        ESS_UINT8 Value = (ESS_UINT8) *pData;

        if(((ESS_UINT16)Value & ReadOnlyMask) == (sDiagHistory.Flags & ReadOnlyMask))
        {
            /*Readonly bits are equal => write new flags value*/    
            sDiagHistory.Flags = (ESS_UINT16)Value;
        }
        else
        {
...
*////////////////////////////////////////////////////////////////////////////////////////
ESS_UINT8 Read0x10F3( ESS_UINT16 index, ESS_UINT8 subindex, ESS_UINT32 dataSize, ESS_UINT16 MBXMEM * pData, ESS_UINT8 bCompleteAccess )
{
    if ( bCompleteAccess )
        /* Complete Access is not supported for object 0x10F3 */
        return ABORTIDX_UNSUPPORTED_ACCESS;
    ...
    if(subindex == 0)
    {
        *pData = (ESS_UINT8)sDiagHistory.u16SubIndex0;
    }
    else if (subindex == 1)
    {
        /* Maximum Messages*/
...
*////////////////////////////////////////////////////////////////////////////////////////
void Diag_CreateNewMessage(ESS_UINT32 DiagCode,ESS_UINT8 Type,ESS_UINT16 TextID,ESS_UINT8 NumParam, TDIAGMSGPARAMINFO *pParam)
{
    ESS_UINT8 MaskedType = Type & DIAG_MSG_TYPE_MASK;
        ...
        || ((MaskedType == DIAG_MSG_TYPE_ERROR) && ((sDiagHistory.Flags & DIAG_DISABLE_ERROR_MSG) == 0))))
    {
        ...
        if((sDiagHistory.Flags & DIAG_OVERWRITE_DISCARD) == 0 || (sDiagHistory.Flags & DIAG_OPERATION_MODE) == 0)
        {
            ...
            {
                ESS_UINT16 ParamLength = 0;
                ESS_UINT8 ParamType = 0;
            ...
            if(aDiagMessages[NextBuffer].pMessage != NULL)
            {
                ...
                pNewDiagMsg->u16Flags = SWAPWORD((((ESS_UINT16)NumParam) << 8) | Type);
...
/*ECATCHANGE_END(V5.11) DIAG1*/
                ...
                if(NumParam > 0 && pParam != NULL)
                {
                    ESS_UINT8 *pMsgParamBuffer = (ESS_UINT8 *)&pNewDiagMsg->Parameter;
                    ESS_UINT16 LengthToCopy = 0;
                    ESS_UINT8 ParamType;
                    pParamWorking = pParam;

                    for(cnt = 0; cnt < NumParam ; cnt++)
                    {
                        ESS_UINT16 u16PFlag = SWAPWORD(pParamWorking->ParamFlags);
