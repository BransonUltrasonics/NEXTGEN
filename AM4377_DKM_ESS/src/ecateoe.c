...
ESS_UINT8               u8ReceiveFragmentNo;                /**< \brief stores the next expected fragment number
                                                            to be received, if the last frame was
                                                            received completely, u8ReceiveFragmentNo is 0 */
ESS_UINT16              u16EthernetReceiveSize;             /**< \brief stores the size of the actual received
                                                            Ethernet frame */
ESS_UINT16              u16EthernetReceiveOffset;           /**< \brief stores the actual offset where the received
                                                            fragment shall be copied to */
ESS_UINT16              u16ReceiveFrameNo;                  /**< \brief store the frameNo, which has to be the same
...
MEM_ADDR MBXMEM *   pEthernetReceiveFrame;              /**< \brief stores the buffer for actual received
                                                            Ethernet frame */
ESS_UINT8               u8SendFragmentNo;                   /**< \brief stores the fragment number of the next fragment to be sent */
ESS_UINT16              u16EthernetSendSize;                /**< \brief stores the size of the actual Ethernet frame to be sent */
ESS_UINT16              u16EthernetSendOffset;              /**< \brief stores the actual offset of the Ethernet frame, which
                                                            points to the next fragment to be sent */
ESS_UINT16              u16SendFrameNo;                     /**< \brief stores the frame number of the actual Ethernet frame to be sent */
...
ESS_UINT8           aEthernetReceiveBuffer[ETHERNET_MAX_FRAMEBUF_LEN];     /**< \brief the PIC18 has not enough memory for dynamic memory
...
ESS_UINT8 EOE_ServiceInd(TMBX MBXMEM *pMbx)
{
    ESS_UINT16 result = 0;
    ...
    ESS_UINT16 expSize = ETHERCAT_EOE_HEADER_LEN;
    ...
    if ( mbxSize >= ETHERCAT_EOE_HEADER_LEN)
    {
        ...
        case EOE_TYPE_FRAME_FRAG:
            ...
            else
            {
                ...
                ESS_UINT16 offset = ((SWAPWORD(pEoe->Flags2) & EOEHEADER_OFFSETBUFFER) >> EOEHEADERSHIFT_OFFSETBUFFER) << 5;    // * 32
                /* get the sent frameNo */
                ESS_UINT16 frameNo = SWAPWORD(pEoe->Flags2) & EOEHEADER_FRAMENO;
            ...
            if ( (u16EthernetReceiveOffset + mbxSize) <= u16EthernetReceiveSize )
            {
                ...
                MBXMEMCPY(&((ESS_UINT8*)pEthernetReceiveFrame)[u16EthernetReceiveOffset],&pEoe[1], mbxSize);
            ...
            if ( SWAPWORD(pEoe->Flags1) & EOEHEADER_LASTFRAGMENT )
            {
                ...
                EOEAPPL_ReceiveFrameInd((ESS_UINT8 *) pEthernetReceiveFrame, u16EthernetReceiveOffset );
                /* next fragment which will be accepted is a the beginning of a new EoE frame (fragmentNo=0) */
                u8ReceiveFragmentNo = 0;
...
void SendFragment(void)
{
    ...
    ESS_UINT16 size = u16EthernetSendSize - u16EthernetSendOffset;
...
/* ECATCHANGE_END(V5.11) EOE2*/
    ...
    psWriteMbx->MbxHeader.Flags[MBX_OFFS_TYPE] = (ESS_UINT16) (MBX_TYPE_EOE << MBX_SHIFT_TYPE);
    ...
    MBXMEMCPY((ESS_UINT8 *)&pEoe[1], &((ESS_UINT8 *)pEthernetSendFrame)[u16EthernetSendOffset], size);
...
ESS_UINT8 EOE_SendFrameReq(ESS_UINT8 MBXMEM * pFrame, ESS_UINT16 frameSize)
{
    if ( !bEoESendFramePending && nAlStatus != STATE_INIT
        && (pEoeSendStored == NULL || pFrame == (ESS_UINT8 MBXMEM *)pEoeSendStored)
#if !MAILBOX_QUEUE        
