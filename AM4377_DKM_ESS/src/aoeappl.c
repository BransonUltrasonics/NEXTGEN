...
ESS_UINT16    AOEAPPL_AmsInd(AmsCmd MBXMEM *pCmd)
{
    ESS_UINT16 result = 0;
    ...
    if (AOE_AmsNetIdsEqual(&pCmd->head.target.netId, &sRouterNetId))
    {
        ...
        ESS_UINT16 port = SWAPWORD(pCmd->head.target.port);
        ESS_UINT16 cmdId = SWAPWORD(pCmd->head.cmdId);
            ...
            if ( cmdId == ADSSRVID_READ || cmdId == ADSSRVID_WRITE )
            {
...
#endif
                ESS_UINT32 dataSize = SWAPDWORD(pCmd->head.cbData);
                ESS_UINT32 indexGroup = SWAPDWORD(pReq->indexGroup);
#if COE_SUPPORTED
                ESS_UINT32 indexOffset = SWAPDWORD(pReq->indexOffset);
#endif
                ESS_UINT32 dataLen = 0;
                    ...
                    )
                {
                    ...
                    case ADSIGRP_CANOPEN_SDO: /* SDO-Request */
                        ...
                        {
                            ESS_UINT16 index = (ESS_UINT16) (indexOffset >> 16);
                            ESS_UINT8 subindex = (ESS_UINT8) (indexOffset & 0xFF);
                            ESS_UINT8 bCompleteAccess = (ESS_UINT8) ((indexOffset >> 8) & 0x01);
                                    ...
                                    if ( dataSize >= dataLen )
                                    {
                                        if ( cmdId == ADSSRVID_READ )
                                        {
                                            ...
                                            if ( pRes != NULL )
                                            {
                                                ...
                                                result = OBJ_Read( index, subindex, dataLen, pObjEntry, (ESS_UINT16 MBXMEM *) &pRes[1], bCompleteAccess );
                                                if ( result == 0 )
                                                {
                                                    // send AoE response
                                                    pRes->cbLength = dataLen;
                                                    pRes->result = 0;
                                                    AOE_AmsRes((AmsCmd MBXMEM *) pRes, result,(ESS_UINT16) dataLen+SIZEOF_TAdsReadRes);
                                                    return result;
                                                }
                                                else
                                        ...
                                        else
                                        {
                                            ...
                                            result = OBJ_Write( index, subindex, dataLen, pObjEntry, (ESS_UINT16 MBXMEM *) &pReq[1], bCompleteAccess );
                                            if ( result == 0 )
                                            {
                                                // send AoE response
                                                pRes->cbLength = dataLen;
                                                pRes->result = 0;
                                                AOE_AmsRes((AmsCmd MBXMEM *) pRes, result,(ESS_UINT16) dataLen+SIZEOF_TAdsWriteRes);
                                                return result;
                                            }
                                            else
                                                result = ADSERR_DEVICE_ERROR;
                    ...
                    case ADSIGRP_CANOPEN_SDO_INFO_LIST:
                            ...
                            if ( dataSize >= (INFO_LIST_TYPE_MAX<<1) )
                            {
                                ...
                                if ( pRes != NULL )
                                {
                                    ...
                                    for (i = 0; i < INFO_LIST_TYPE_MAX; i++)
                                    {
                                        ...
                                        ((ESS_UINT16 MBXMEM *) &pRes[1])[i+1] = SWAPWORD(n);
                                    }
                        ...
                        {
                            ESS_UINT32 n = OBJ_GetNoOfObjects((ESS_UINT8) ((indexOffset >> 16) - 1));

                            if ( dataSize >= (n<<1) )
                            {
                                /* get buffer for response */
                                pRes = (TAdsReadRes MBXMEM *) ALLOCADSMEM(SIZEOF(TAdsReadRes) + (n<<1) + SIZEOF(ESS_UINT16) + SIZEOF_SDOINFOLISTHEAD);
                                if ( pRes != NULL )
                                {
                                    TSDOINFOLIST MBXMEM *pListRes = (TSDOINFOLIST MBXMEM *) &pRes[1];
                                    ESS_UINT16 MBXMEM *pList = (ESS_UINT16 MBXMEM *) &pListRes[1];
                                    // start with the first possible index
                                    ESS_UINT16 index = 0x1000;

                                    /* copy AMS header */
                                    HMEMCPY(pRes, pReq, SIZEOF(AmsHead));
                                    pListRes->ListType = (indexOffset >> 16);
                                    OBJ_GetObjectList(pListRes->ListType-1, &index, (ESS_UINT16) (n<<1), pList,NULL);
                                    // send AoE response
                            ...
                            if ( pObjEntry )
                            {
                                ...
                                if ( dataSize >= SDO_INFO_OBJ_DESC_RES_SIZE )
                                {
                                    ...
                                    if ( pRes != NULL )
                                    {
                                        ...
                                        {
                                            OBJ_GetDesc(index, 0, pObjEntry, ((ESS_UINT16 MBXMEM *) &(&pObj->Res)[1]));
                                        }
                                ...
                                if ( subindex <= maxSubindex )
                                {
                                    ...
                                    if ( dataSize >= SIZEOF(TSDOINFOENTRY) )
                                    {
                                        ...
                                        if ( pRes != NULL )
                                        {
                                            ...
                                            ((ESS_UINT16 MBXMEM *) &pRes[1])[1] = (ESS_UINT16) indexOffset;
#if BIG_ENDIAN_FORMAT
                                            {
                                            ...
                                            if ( dataLen > SDO_INFO_OBJ_DESC_RES_SIZE )
                                            {
                                                OBJ_GetDesc(index, subindex, pObjEntry, ((ESS_UINT16 MBXMEM *) &(&pEntry->Res)[1]));
                                            }
                ...
                AOE_AmsRes(pCmd, 0,(ESS_UINT16) dataLen);
                return result;
            }
            else
