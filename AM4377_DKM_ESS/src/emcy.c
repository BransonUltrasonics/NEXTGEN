...
#if COE_SUPPORTED
const ESS_UINT16 MBXMEM acEmcyCoeHeader[4] = {COE_HEADER_SIZE + EMCY_SIZE, 0, SWAPWORD(MBX_TYPE_COE<<8), SWAPWORD(COESERVICE_EMERGENCY<<12)};
#define    MAX_EMCY_SIZE    (SIZEOF(acEmcyCoeHeader)+SIZEOF(TEMCYMESSAGE))

#endif
#if SOE_SUPPORTED
const ESS_UINT16 MBXMEM acEmcySoeHeader[3] = {SOE_HEADER_SIZE + EMCY_SIZE, 0, SWAPWORD(MBX_TYPE_SOE<<8)};
const ESS_UINT16 MBXMEM acNotiSoeHeader[3] = {SOE_HEADER_SIZE + SIZEOF(ESS_UINT16), 0, SWAPWORD(MBX_TYPE_SOE<<8)};
...
{
    ESS_UINT16 lastInQueue = sEmptyEmcyQueue.LastInQueue+1;
...
TEMCYMESSAGE EMCYMEM * GetOutOfEmptyEmcyQueue(void)
{
    ...
    {
        ESS_UINT16 firstInQueue = sEmptyEmcyQueue.FirstInQueue;
...
TEMCYMESSAGE EMCYMEM * GetOutOfSendEmcyQueue(void)
{
    ...
    {
        ESS_UINT16 firstInQueue = sSendEmcyQueue.FirstInQueue;
...
ESS_UINT8 EMCY_IsQueueEmpty(void)
{
...
ESS_UINT8 EMCY_SendEmergency( TEMCYMESSAGE EMCYMEM *pEmcy )
{
    // HBu 02.05.06: when using the mailbox event in an ISR it should be disabled here
    DISABLE_MBX_INT;
