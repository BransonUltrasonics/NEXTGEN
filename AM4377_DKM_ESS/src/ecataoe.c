...
typedef struct
{
    ESS_UINT16    firstInQueue; /**< \brief First in queue*/
    ESS_UINT16    lastInQueue; /**< \brief last in queue*/
    ESS_UINT16    maxQueueSize; /**< \brief Queue size*/
...
typedef struct
{
    ESS_UINT16                packetNo; /**< \brief Packet number*/
    ESS_UINT16                fragmentNo; /**< \brief Fragment number*/
    AmsAddr                sender; /**< \brief Sender AMS address*/
    ESS_UINT32                actOffs; /**< \brief Actual offset*/
    ESS_UINT16                ttl; /**< \brief Ttl*/
...
ESS_UINT16                    u16AmsFragAck; /**< \brief AMS fragment acknowledge*/
ESS_UINT16                    u16AmsPacketNo; /**< \brief AMS Packet number*/
ESS_UINT16                    u16AmsCmdsInWork; /**< \brief AMS command in work*/
...
------
---------------------------------------------------------------------------------------*/
ESS_UINT16 AmsFragRes(AmsCmd MBXMEM *pCmd, ESS_UINT16 dataLen);
ESS_UINT16 InsertInFragTable(AmsCmd MBXMEM *pCmd);
AmsCmd MBXMEM * RemoveFromFragTable(ESS_UINT16 index);
AmsCmd MBXMEM * AoeGetOutOfQueue(void);
ESS_UINT16 AoePutInQueue(AmsCmd MBXMEM *pCmd);
...
ESS_UINT16 AmsFragRes(AmsCmd MBXMEM *pCmd, ESS_UINT16 dataLen)
{
    ESS_UINT16 result = ERR_NOERROR;
    ESS_UINT16 maxDataLen = u16AmsFragSize;

    if (maxDataLen < dataLen)
    {
        /* command has to fragmented because it doesn't fit in the mailbox */
        ESS_UINT8        bLoop = 1;
        ESS_UINT16    offset = 2 * SIZEOF(AmsAddr);
        ESS_UINT16    fragNo = 0;
        ...
        while (bLoop)
        {
            ...
            if (pMbx)
            {
                ...
                HMEMCPY((ESS_UINT8 MBXMEM *) pFragCmd, (ESS_UINT8 MBXMEM *) pCmd, 2*SIZEOF(AmsAddr));
                ...
                fragNo++;
                HMEMCPY(&pFragCmd[1], &((ESS_UINT8 MBXMEM *) pCmd)[offset], size);
                offset += size;
                dataLen -= size;
    ...
    if (pCmd->head.hUser.frag.fragmentNo == 0)
    {
        if (pCmd->head.cbData >= SIZEOF(AmsFirstFragmentHead))
        {
            ...
            if (u16AmsCmdsInWork < MAX_NO_AMS_CMD_IN_WORK)
            {
                ...
                if (pFullCmd)
                {
                    HMEMCPY((ESS_UINT8 MBXMEM *) pFullCmd, (ESS_UINT8 MBXMEM *) pCmd, 2*SIZEOF(AmsAddr));
                    HMEMCPY((ESS_UINT8 MBXMEM *) &pFullCmd->head.cmdId, (ESS_UINT8 MBXMEM *) &pHead->org, pCmd->head.cbData);
                    HMEMCPY((ESS_UINT8 MBXMEM *) &pEntry->sender, (ESS_UINT8 MBXMEM *) &pCmd->head.sender, SIZEOF(AmsAddr));
...
AmsCmd MBXMEM * RemoveFromFragTable(ESS_UINT16 index)
{
    ...
    if (index < u16AmsCmdsInWork)
    {
        for (i = index; i < (u16AmsCmdsInWork - 1); i++)
        {
            ...
            HMEMCPY((ESS_UINT8 MBXMEM *) &asAmsCmdsInWork[i], (ESS_UINT8 MBXMEM *) &asAmsCmdsInWork[i + 1], SIZEOF(TAmsFragTable));
        }
...
AmsCmd MBXMEM * AOE_FragmentedCmdInd(AmsCmd MBXMEM *pCmd)
{
    ...
    for (i = 0; (i < u16AmsCmdsInWork)&&(!bFound); i++)
    {
        ...
        ESS_UINT8 NetIdEqual = AOE_AmsNetIdsEqual(&pCmd->head.sender.netId, &pEntry->sender.netId);
            ...
            )
        {
            ...
            if (pCmd->head.hUser.frag.fragmentNo == pEntry->fragmentNo)
            {
                ...
                {
                    bFound = 1;
                    HMEMCPY(&((ESS_UINT8 MBXMEM *)pFullCmd)[pEntry->actOffs], &pCmd[1], pCmd->head.cbData);
...
ESS_UINT8 AOE_AmsNetIdsEqual(AmsNetId MBXMEM *pNetId1, AmsNetId MBXMEM *pNetId2)
{
    int i = 0;
...
ESS_UINT16 AOE_AmsRes(AmsCmd MBXMEM *pCmd, ESS_UINT16 amsErrCode, ESS_UINT16 dataLen)
{
    AmsAddr amsAddr;

    /* change Sender and Target */
    HMEMCPY((ESS_UINT8 MBXMEM *) &amsAddr, (ESS_UINT8 MBXMEM *) &pCmd->head.sender, SIZEOF(AmsAddr));
    HMEMCPY((ESS_UINT8 MBXMEM *) &pCmd->head.sender, (ESS_UINT8 MBXMEM *) &pCmd->head.target, SIZEOF(AmsAddr));
    HMEMCPY((ESS_UINT8 MBXMEM *) &pCmd->head.target, (ESS_UINT8 MBXMEM *) &amsAddr, SIZEOF(AmsAddr));
...
AmsCmd MBXMEM * AoeGetOutOfQueue(void)
{
    ...
    {
        ESS_UINT16 firstInQueue = sAdsIndQueue.firstInQueue;
...
ESS_UINT8 AOE_ServiceInd(TMBX MBXMEM *pMbx)
{
    ESS_UINT16 mbxSize = SWAPWORD(pMbx->MbxHeader.Length);
    ...
    if (mbxSize)
    {
        ...
        {
            HMEMCPY((ESS_UINT8 MBXMEM *) pCmd, pMbx->Data, (mbxSize+1) & 0xFFFE);
#if MAILBOX_QUEUE
            APPL_FreeMailboxBuffer(pMbx);
#endif
            if ( pCmd->head.cmdId == ADSSRVID_WRITE && pCmd->head.cbData >= (SIZEOF(TAdsWriteReq)-SIZEOF(AmsHead)+SIZEOF(AmsNetId)) )
            {
                ...
                ecatAddr = (ESS_UINT16) ((SWAPDWORD(TmpVal) & ESC_SLAVE_ADDRESS_MASK));
#else
                   ...
                   )
                {
                    ...
                    if (((ESS_UINT32 MBXMEM *) &pReq[1])[0] == 0)
                    {
                        // relative Net-ID
