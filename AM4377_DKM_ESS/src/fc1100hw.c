...
------
-----------------------------------------------------------------------------------------*/
ESS_BOOL InitSystemEvents(void);
void FreeTimerResources(void);


ESS_UINT16* GetFc1100Pointer(void)
{
    ...
    pFc1100Device = (ESS_UINT16*)MapESCPointer(1);
#ifdef _DEBUG
...
ESS_BOOL WINAPI ConsoleHandler(DWORD CEvent)
{
    switch(CEvent)
    {
...
ESS_BOOL InitSystemEvents(void)
{
    ...
    if(Result == TRUE)
        Result = (ESS_BOOL) SetConsoleCtrlHandler((PHANDLER_ROUTINE)ConsoleHandler, TRUE);
...
#if UC_SET_ECAT_LED
void HW_SetLed(ESS_BOOL RunLed,ESS_BOOL ErrLed)
{
