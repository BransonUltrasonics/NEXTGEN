...
/* ECATCHANGE_START(V5.11) MBX2*/
    ESS_BOOL bNoMbxMemoryAvailable; /**< \brief Indicates if enough dynamic memory is available to handle at least one mailbox datagram */
...
ESS_UINT8 PutInMbxQueue(TMBX MBXMEM * pMbx, TMBXQUEUE MBXMEM * pQueue)
{
...
TMBX MBXMEM * GetOutOfMbxQueue(TMBXQUEUE MBXMEM * pQueue)
{
    ...
    if (pQueue->firstInQueue != pQueue->lastInQueue)
    {
        ...
        ESS_UINT16 firstInQueue = pQueue->firstInQueue;
...
*////////////////////////////////////////////////////////////////////////////////////////
ESS_UINT16 MBX_StartMailboxHandler(void)
{
    ESS_UINT16 result = 0;
...
/*ECATCHANGE_END(V5.11) HW1*/
    ...
    u16ReceiveMbxSize     = (ESS_UINT16) ((pSyncMan->AddressLength & SM_LENGTH_MASK) >>SM_LENGTH_SHIFT);
    /* store the address of the receive mailbox */
    u16EscAddrReceiveMbx = (ESS_UINT16) (pSyncMan->AddressLength & SM_ADDRESS_MASK);
#else
    /* store size of the receive mailbox */
...
/*ECATCHANGE_END(V5.11) HW1*/
    ...
    u16SendMbxSize = (ESS_UINT16) ((pSyncMan->AddressLength & SM_LENGTH_MASK) >>SM_LENGTH_SHIFT);
    /* store the address of the send mailbox */
    u16EscAddrSendMbx = (ESS_UINT16) (pSyncMan->AddressLength & SM_ADDRESS_MASK);
#else
    /* store the size of the send mailbox */
...
ESS_UINT8 MailboxServiceInd(TMBX MBXMEM *pMbx)
{
    ESS_UINT8 result;
...
{
    ESS_UINT8 result = 0;
    ESS_UINT8 mbxCounter = pMbx->MbxHeader.Flags[MBX_OFFS_COUNTER] >> MBX_SHIFT_COUNTER;
    ESS_UINT16 MbxLen = SWAPWORD(pMbx->MbxHeader.Length);
...
ESS_UINT8 MBX_MailboxSendReq( TMBX MBXMEM * pMbx, ESS_UINT8 flags )
{
    ESS_UINT8 result = 0;
...
/*ECATCHANGE_START(V5.11) TEST4*/
        if(IS_TEST_ACTIVE(Mbx_AlternatingMbxCntChange))
        {
            ...
            ESS_UINT8 timerval = (ESS_UINT8)HW_GetTimer();
...
/*ECATCHANGE_END(V5.11) TEST4*/
    ...
    if ( flags & FRAGMENTS_FOLLOW )
    {
        ...
        u8MailboxSendReqStored |= (flags & ((ESS_UINT8) ~FRAGMENTS_FOLLOW));
    }
...
#if ESC_32BIT_ACCESS
        VARVOLATILE ESS_UINT32 smstate = 0x00;
...
ESS_UINT8 MBX_CopyToSendMailbox( TMBX MBXMEM *pMbx )
{
    if ( (nAlStatus & STATE_MASK) == STATE_INIT)
    {
    ...
    else
    {
        ...
        ESS_UINT16 mbxSize = pMbx->MbxHeader.Length;
#if BIG_ENDIAN_FORMAT
...
/*ECATCHANGE_END(V5.11) ECAT7*/
        ...
        if(!(smstate & SM_STATUS_MBX_BUFFER_FULL))
        {
            ...
            ESS_UINT8 BytesLeft = u16SendMbxSize - (mbxSize + MBX_HEADER_SIZE);
#endif
            ...
            if(BytesLeft < 4)
            {
                ...
                ESS_UINT32 LastDataAddress = ((mbxSize + MBX_HEADER_SIZE)/4)*4;
            
                /*Copy the buffer to overwrite*/
                MEMCPY((ESS_UINT32 *)&u32dummy,(((ESS_UINT8 *)pMbx) + LastDataAddress),(4 - BytesLeft));
            }
            ...
            if(BytesLeft < 2)
            {
                ...
                ESS_UINT32 LastDataAddress = ((mbxSize + MBX_HEADER_SIZE)/2)*2;
            
                /*Copy the buffer to overwrite*/
                MEMCPY((ESS_UINT16 *)&u16dummy,(((ESS_UINT8 *)pMbx) + LastDataAddress),(2 - BytesLeft));
            }
