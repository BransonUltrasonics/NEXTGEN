...
ESS_UINT8 SOE_ServiceInd(TMBX MBXMEM *pReceiveMbx)
{
    ESS_UINT8 result = 0;
...
{
    ESS_UINT32 data=0x12345678;
    pSoeMbx->SoeHeader.Flags.Word &= ~SOEFLAGS_OPCODE;
    pSoeMbx->SoeHeader.Flags.Word |= ECAT_SOE_OPCODE_RRS;
    MBXMEMCPY( ( ESS_UINT8 MBXMEM * ) &pSoeMbx->Data[0], ( ESS_UINT8 MBXMEM * ) &data, SIZEOF( data ) );
...
void SOE_WriteInd(TSOEMBX MBXMEM *pSoeMbx)
{
    ...
    if ( dataLen < SIZEOF(ESS_UINT32) )
        errorCode = 0x7002;
    else if ( dataLen > SIZEOF(ESS_UINT32) )
        errorCode = 0x7003;
    if ( errorCode != 0 )
    {
        pSoeMbx->SoeHeader.Flags.Word |= SOEFLAGS_ERROR;
        pSoeMbx->MbxHeader.Length      += SIZEOF( errorCode );
        MBXMEMCPY( (ESS_UINT8 MBXMEM *) &pSoeMbx->Data[0], ( ESS_UINT8 MBXMEM * ) &errorCode, SIZEOF(errorCode) );
    }
    MBX_MailboxSendReq( (TMBX MBXMEM *) pSoeMbx,SOE_SERVICE);
}
...
ESS_UINT16 SOE_NotificationReq(ESS_UINT16 idn, ESS_UINT16 channel, ESS_UINT16 dataState)
{
#if EMERGENCY_SUPPORTED
