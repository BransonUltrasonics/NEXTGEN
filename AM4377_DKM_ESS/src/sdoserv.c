...
extern ESS_UINT16 OBJ_GetDesc( ESS_UINT16 index, ESS_UINT8 subindex, OBJCONST TOBJECT OBJMEM * pObjEntry, ESS_UINT16 MBXMEM * pData );
extern OBJCONST TSDOINFOENTRYDESC OBJMEM * OBJ_GetEntryDesc(OBJCONST TOBJECT OBJMEM * pObjEntry, ESS_UINT8 Subindex);
...
const ESS_UINT32 MBXMEM cAbortCode[] =
{
    ABORT_NOERROR,
...
ESS_UINT16 VARMEM                            nSdoInfoIndex;
OBJCONST TOBJECT OBJMEM * VARMEM        pSdoInfoObjEntry;

#if SDO_RES_INTERFACE
TINITSDOMBX MBXMEM *    VARMEM            pSdoResStored;
ESS_BOOL    VARMEM                            bSdoInWork = FALSE;
#endif

#if SEGMENTED_SDO_SUPPORTED
ESS_UINT8    VARMEM                         nSdoSegService;
ESS_UINT8    VARMEM                         bSdoSegFollows;
ESS_UINT8    VARMEM                         bSdoSegAccess;
ESS_UINT16 VARMEM                           nSdoSegIndex;
ESS_UINT8 VARMEM                            nSdoSegSubindex;
ESS_UINT32 VARMEM                           nSdoSegBytesToHandle;
ESS_UINT8 VARMEM                            bSdoSegLastToggle;
ESS_UINT32 VARMEM                           nSdoSegCompleteSize;
OBJCONST TOBJECT OBJMEM * VARMEM        pSdoSegObjEntry;

#if _PIC18
ESS_UINT8                                TmpBuffer[256];
ESS_UINT8 *                            GetTmpBuffer(int x)
{
    return TmpBuffer;
}
...
---------------------------------------------------------------------------------------*/
#if SEGMENTED_SDO_SUPPORTED
static ESS_UINT8 SdoDownloadSegmentInd(TDOWNLOADSDOSEGREQMBX MBXMEM * pSdoInd);
static ESS_UINT8 SdoUploadSegmentInd(TUPLOADSDOSEGREQMBX MBXMEM * pSdoInd);
#endif
/*---------------------------------------------------------------------------------------
------
------    Functions
...
static ESS_UINT8 SdoDownloadSegmentInd( TDOWNLOADSDOSEGREQMBX MBXMEM * pSdoInd )
{
    ...
    else
    {
        ...
        ESS_UINT16 maxData =    u16ReceiveMbxSize - MBX_HEADER_SIZE - SEGMENT_NORM_HEADER_SIZE;
...
/* ECATCHANGE_START(V5.11) ECAT7*/
                    ESS_UINT16 mbxSize = SWAPWORD(pSdoInd->MbxHeader.Length);
                    ...
                    if (((bytesToSave <= (ESS_UINT32)(mbxSize - SEGMENT_NORM_HEADER_SIZE))
                         &&( bytesToSave == ((ESS_UINT16) (MIN_SEGMENTED_DATA - ((pSdoInd->SdoHeader.SegHeader & SEGHEADER_SEGDATASIZE) >> SEGHEADERSHIFT_SEGDATASIZE))) )
                          )
...
/* ECATCHANGE_END(V5.11) ECAT7*/
...
#else
                MBXMEMCPY( ((ESS_UINT8*)pSdoSegData) + (nSdoSegBytesToHandle), pSdoInd->SdoHeader.Data, bytesToSave ); 
#endif

                if ( bSdoSegFollows == FALSE    )
                {
                    /* it was the last segment, OBJ_Write will called to make the Write-operation */
                    abort = OBJ_Write( nSdoSegIndex, nSdoSegSubindex, nSdoSegCompleteSize, pSdoSegObjEntry, (ESS_UINT16 MBXMEM *) pSdoSegData, bSdoSegAccess );
#if SDO_RES_INTERFACE
...
#endif // SDO_RES_INTERFACE
                    {
                        ...
                        FREEMEM( (ESS_UINT16 VARMEM *) pSdoSegData );
                        pSdoSegData = NULL;
                    }
                }
            }
    ...
    if ( abort == 0)
    {
        ...
        pSdoInd->CoeHeader |= ((ESS_UINT16)COESERVICE_SDORESPONSE) << COEHEADER_COESERVICESHIFT;
...
static ESS_UINT8 SdoUploadSegmentInd( TUPLOADSDOSEGREQMBX MBXMEM * pSdoInd )
{
   ...
   if ( (pSdoInd->SegHeader & SEGHEADER_TOGGLE) == bSdoSegLastToggle )
    ...
    else
    {
        ...
        {	
            ESS_UINT16 i = 0;
            ESS_UINT32 nIndexOffset = nSdoSegBytesToHandle >> 1;
...
#else
        MBXMEMCPY( pSdoSegRes->SdoHeader.Data, &(((ESS_UINT8*)pSdoSegData)[nSdoSegBytesToHandle]), size );
#endif
        
        /* the SDO Upload Segment header depends if there is still data to be sent */
        pSdoSegRes->CoeHeader &= ~COEHEADER_COESERVICEMASK;
        pSdoSegRes->CoeHeader |= ((ESS_UINT16)COESERVICE_SDORESPONSE) << COEHEADER_COESERVICESHIFT;
        ...
        {
            pSdoSegRes->MbxHeader.Length         = ((ESS_UINT16) size) + SEGMENT_NORM_HEADER_SIZE;
        }
...
void SdoRes(ESS_UINT8 abort, ESS_UINT8 command, ESS_UINT8 completeAccess, ESS_UINT16 dataSize, ESS_UINT32 objLength, TINITSDOMBX MBXMEM *pSdoRes)
{
    ...
    if ( abort == 0 )
    {
        ...
        pSdoRes->CoeHeader |= ((ESS_UINT16)COESERVICE_SDORESPONSE) << COEHEADER_COESERVICESHIFT;
        if ( command == SDOSERVICE_INITIATEUPLOADREQ )
        {
            ...
            if ( (objLength <= 4) && (objLength > 0) )
            {
                ...
                pSdoRes->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]    |=     SDOHEADER_SIZEINDICATOR     |
                                                                                        ...
                                                                                        completeAccess |
                                                                                        ((MAX_EXPEDITED_DATA - ((ESS_UINT8)objLength)) << SDOHEADERSHIFT_DATASETSIZE) |
...
#endif
                {
                    pSdoRes->MbxHeader.Length         =         UPLOAD_NORM_RES_SIZE+((ESS_UINT16)objLength);
                }
                   ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoRes)->CompleteSize[0] = SWAPWORD((ESS_UINT16)objLength);
                   ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoRes)->CompleteSize[1] = SWAPWORD((ESS_UINT16)(objLength>>16));
                pSdoRes->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]    |=     SDOHEADER_SIZEINDICATOR     |
                                                                                        completeAccess |
                                                                                        SDOSERVICE_INITIATEUPLOADRES;
            }
    ...
    else
    {
        ...
        pSdoRes->CoeHeader &= ~COEHEADER_COESERVICEMASK;
        pSdoRes->CoeHeader |= ((ESS_UINT16)COESERVICE_SDOREQUEST) << COEHEADER_COESERVICESHIFT;
        pSdoRes->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]    = SDOSERVICE_ABORTTRANSFER;
#if BIG_ENDIAN_16BIT
        ((ESS_UINT16 MBXMEM *) &((TABORTSDOTRANSFERREQMBX MBXMEM *) pSdoRes)->AbortCode)[0] = ((ESS_UINT16 *) &cAbortCode[abort])[1];
        ((ESS_UINT16 MBXMEM *) &((TABORTSDOTRANSFERREQMBX MBXMEM *) pSdoRes)->AbortCode)[1] = ((ESS_UINT16 *) &cAbortCode[abort])[0];
#else
...
ESS_UINT8 SDOS_SdoInd(TINITSDOMBX MBXMEM *pSdoInd)
{
    ESS_UINT8 abort = 0;
    ESS_UINT8 sdoHeader = pSdoInd->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] & SDOHEADER_COMMANDMASK;
    /* the SDO-command is in bit 5-7 of the first SDO-Byte */
    ESS_UINT8 command = (sdoHeader & SDOHEADER_COMMAND);
    ...
    ESS_UINT8 bCompleteAccess = 0;
    ESS_UINT32 objLength = 0;
    ESS_UINT32 dataSize = 0;
    ...
    case SDOSERVICE_INITIATEUPLOADREQ:
        ...
        if ( pObjEntry )
        {
            ...
            ESS_UINT8 bTransferType = 0;
            /* pData is the pointer to the received (SDO-Download) or sent (SDO-Upload) SDO data in the mailbox */
            ESS_UINT16 MBXMEM * pData = NULL;
#if SEGMENTED_SDO_SUPPORTED
            ESS_UINT8 segTransfer = 0;
#endif

            {
                dataSize = objLength = OBJ_GetObjectLength( index, subindex, pObjEntry, (ESS_UINT8) (sdoHeader & SDOHEADER_COMPLETEACCESS) );

                if ( command == SDOSERVICE_INITIATEUPLOADREQ )
                {
#ifdef WELD_RECIPE_CONSOLE_LOGGING
                	if(index == WELD_RECIPE_OBJ_IND)
                    {
                		printf("\nEcSlave:SDO INITIATE UPLOAD REQUEST\n\n");
                    }
#endif
                    /* SDO Upload */
            ...
            if ( command == SDOSERVICE_INITIATEDOWNLOADREQ )
            {
            	...
            	if(index == WELD_RECIPE_OBJ_IND)
            	{
            		printf("\nEcSlave:SDO INITIATE DOWNLOAD REQUEST");
            	}
#endif
                /* SDO Download */
                if ( bTransferType )
                {
                    ...
                    pData = (ESS_UINT16 MBXMEM *) &pSdoInd[1];
                }
                else
                {
                    /* Normal Download */
                    ...
                    ESS_UINT32 downloadSize = ((ESS_UINT32)(SWAPWORD(((TINITSDODOWNLOADNORMREQMBX MBXMEM *) pSdoInd)->CompleteSize[1]))<<16) + (SWAPWORD(((TINITSDODOWNLOADNORMREQMBX MBXMEM *) pSdoInd)->CompleteSize[0]));
                    ...
                    /* pData is the pointer to the downloaded object data */
                    pData = (ESS_UINT16 MBXMEM *) ((TINITSDODOWNLOADNORMREQMBX MBXMEM *) pSdoInd)->Data;
                    ...
                    if ( dataSize > (ESS_UINT32)(mbxSize - DOWNLOAD_NORM_REQ_SIZE) )
#if SEGMENTED_SDO_SUPPORTED
                        /* Segmented Download */
                        segTransfer = 1;
            ...
            if ( abort == 0 )
            {
                ...
                if ( segTransfer )
                {
                    ...
                    pSdoSegData = (ESS_UINT16 VARMEM *) ALLOCMEM( ROUNDUPBYTE2WORD(nSdoSegCompleteSize) );
...
/*ECATCHANGE_END(V5.11) SDO4*/
                    ...
                    else
                    {
                        ...
                        {
                            /* Segmented Upload */
                            abort = OBJ_Read( index, subindex, objLength, pObjEntry, (ESS_UINT16 MBXMEM *) pSdoSegData, bCompleteAccess );
                            if ( abort == 0 )
                            {
...
#endif // SEGMENTED_SDO_SUPPORTED
                {
                    ...
                    else
                    {
                        ...
                        if (0x0 == abort)
                        {
                        	if(index == WELD_RECIPE_OBJ_IND)
                        	{
                        	       SDO_ConsoleLogging(subindex , bCompleteAccess);
                        	}
                        }
#endif
...
#endif
    {
        ...
        SdoRes(abort, command, (ESS_UINT8) (sdoHeader & SDOHEADER_COMPLETEACCESS), (ESS_UINT16) dataSize, objLength, pSdoInd);
    }
...
void SDOS_SdoRes(ESS_UINT8 abort, ESS_UINT32 objLength, ESS_UINT16 MBXMEM *pData)
{
    ESS_UINT16 dataSize = 0;

    if (bSdoInWork)
    {
        /* SDO-Response is expected */
        ESS_UINT8 command = pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] & SDOHEADER_COMMAND;
        ESS_UINT8 completeAccess = pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] & SDOHEADER_COMPLETEACCESS;
        ...
        if ( command == SDOSERVICE_INITIATEUPLOADREQ )
        {
            ...
            if ( dataSize < objLength )
            {
                ...
                nSdoSegCompleteSize    = objLength;
                nSdoSegService            = SDOSERVICE_UPLOADSEGMENTREQ;
                pSdoSegData                = (ESS_UINT16 VARMEM *) pData;
...
ESS_UINT8 SDOS_SdoInfoInd( TSDOINFORMATION MBXMEM *pSdoInfoInd )
{
    ESS_UINT8 abort = 0;
    /* the variable opCode contains the requested SDO Information type */
    ESS_UINT8 opCode = (ESS_UINT8) ((pSdoInfoInd->SdoHeader.InfoHead & INFOHEAD_OPCODE_MASK) >> INFOHEAD_OPCODE_SHIFT);
    OBJCONST TOBJECT OBJMEM * pObjEntry;
    ESS_UINT16 index;
    ESS_UINT8 flags = COE_SERVICE;
    ...
    case SDOINFOSERVICE_OBJDICTIONARYLIST_Q:
        ...
        if ( SWAPWORD(pSdoInfoInd->SdoHeader.Data.List.ListType) <= INFO_LIST_TYPE_MAX )
        {
            ...
            ESS_UINT8 listType = (ESS_UINT8) SWAPWORD(pSdoInfoInd->SdoHeader.Data.List.ListType);
            ...
            if ( listType-- == 0 )
            {
                ...
                {
                    ESS_UINT16 n = OBJ_GetNoOfObjects(i);

                    /* copy the number of objects of the list type in the SDO Information response */
                    ((ESS_UINT16 MBXMEM *) &pSdoInfoInd->CoeHeader)[(SIZEOF_SDOINFOLISTSTRUCT>>1)+i] = SWAPWORD(n);
                }
            ...
            else
            {
                ...
                if ( nSdoInfoFragmentsLeft )
                {
                    ...
                    pData = &((ESS_UINT16 MBXMEM *) &pSdoInfoInd->CoeHeader)[SIZEOF_SDOINFO>>1];
...
#endif
                {
                    ...
                    /* initialize pData with the pointer where the fragment has to be copied */
                    pData = &((ESS_UINT16 MBXMEM *) &pSdoInfoInd->CoeHeader)[SIZEOF_SDOINFOLISTSTRUCT>>1];
            ...
            if(abort == 0)
            {
             ...
             pSdoInfoInd->SdoHeader.InfoHead |= (ESS_UINT16) (SDOINFOSERVICE_OBJDICTIONARYLIST_S << INFOHEAD_OPCODE_SHIFT);
#if !SEGMENTED_SDO_SUPPORTED
                ...
                if (nSdoInfoFragmentsLeft)
                {
                    ...
                    pSdoInfoInd->SdoHeader.InfoHead |= (ESS_UINT16) (SDOINFOSERVICE_INCOMPLETE << INFOHEAD_OPCODE_SHIFT);
        ...
        else
        {
            ...
            if ( pObjEntry )
            {
                ...
                if ( opCode == SDOINFOSERVICE_OBJDESCRIPTION_Q )
                {
                    ...
                    {
                        ESS_UINT16 Flags = SWAPWORD(pSdoInfoInd->SdoHeader.Data.Obj.Res.ObjFlags);

                        Flags &= ~OBJFLAGS_OBJCODEMASK;
                        Flags |= ((ESS_UINT16) OBJCODE_REC) << OBJFLAGS_OBJCODESHIFT;
                    ...
                    else
                    {
                        ...
                        size = OBJ_GetDesc(index, 0, pObjEntry, ((ESS_UINT16 MBXMEM *) &(&pSdoInfoInd->SdoHeader.Data.Obj.Res)[1])) + SIZEOF_SDOINFOOBJSTRUCT;
                    }
                }
                else
                {
                    ...
                    ESS_UINT8 subindex = (ESS_UINT8) ((pSdoInfoInd->SdoHeader.Data.Entry.Info & ENTRY_MASK_SUBINDEX) >> ENTRY_SUBINDEX_SHIFT);

                    /* get the maximum subindex */
                    ESS_UINT8 maxSubindex = (OBJ_GetObjDesc(pObjEntry)->ObjFlags & OBJFLAGS_MAXSUBINDEXMASK) >> OBJFLAGS_MAXSUBINDEXSHIFT;

                    if ( subindex <= maxSubindex )
                    {
                        ESS_UINT16 ObjectFlags;
                        ...
                        {
                            OBJTOMBXSTRCPY( ((ESS_UINT16 MBXMEM *) &(&pSdoInfoInd->SdoHeader.Data.Entry.Res)[1]), aSubindexDesc, SIZEOF(aSubindexDesc) );
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                size = OBJ_GetDesc(index, subindex, pObjEntry, ((ESS_UINT16 MBXMEM *) &(&pSdoInfoInd->SdoHeader.Data.Entry.Res)[1])) + SIZEOF_SDOINFO + SIZEOF(TSDOINFOENTRY);
                            }
                        }
                    }
                    else
                ...
                {
                    {
                        ...
                        pSdoInfoInd->SdoHeader.InfoHead &= ~INFOHEAD_OPCODE_MASK;
                        pSdoInfoInd->SdoHeader.InfoHead |= (ESS_UINT16)((opCode + 1) << INFOHEAD_OPCODE_SHIFT);
                    }
                }
            }
            else
    ...
    if ( abort )
    {
        ...
        pSdoInfoInd->SdoHeader.InfoHead &= ~INFOHEAD_OPCODE_MASK;
        pSdoInfoInd->SdoHeader.InfoHead |= (ESS_UINT16) ((SDOINFOSERVICE_ERROR_Q) << INFOHEAD_OPCODE_SHIFT);

        pSdoInfoInd->SdoHeader.FragmentsLeft = 0;
#if BIG_ENDIAN_16BIT
        ((ESS_UINT16 MBXMEM *) &pSdoInfoInd->SdoHeader.Data.Error.ErrorCode)[0] = ((const ESS_UINT16 *) &cAbortCode[abort])[1];
        ((ESS_UINT16 MBXMEM *) &pSdoInfoInd->SdoHeader.Data.Error.ErrorCode)[1] = ((const ESS_UINT16 *) &cAbortCode[abort])[0];
#else
...
/////////////////////////////////////////////////////////////////////////////////////////
/**
 \param    	printVar	   buffer to which console log is stored for printing later in PDItask

 	 	 	 subIndex      subIndex for which SDO request received.

 \brief    Function prints onto console the content at sub Index of the Weld Recipe object
 	 	 	 for which SDO Upload / Download request received.
*////////////////////////////////////////////////////////////////////////////////////////
static void weldRecipeFieldConsoleLog(ESS_INT8* printVar , ESS_UINT8 subIndex)
{
#if !defined(POWER_CONTROLLER_BUILD) && !defined(ACTUATION_CONTROLLER_BUILD)
	switch(subIndex)
	{
	case 0x0:
		sprintf(printVar, "Num Of Entries = 0x%x",sWeldRecipeParams.u16subIndex0);
		break;
	case 0x01:
		sprintf(printVar , "Weld Delay = 0x%x",sWeldRecipeParams.WeldDelay);
		break;
	case 0x02:
		sprintf(printVar , "Weld Time  = 0x%lx",sWeldRecipeParams.WeldTime);
		break;
	case 0x03:
		sprintf(printVar , "Intensity  = 0x%x",sWeldRecipeParams.Intensity);
		break;
	case 0x04:
		sprintf(printVar , "Control Recipe = 0x%x",sWeldRecipeParams.ControlRecipe);
		break;
	case 0x05:
		sprintf(printVar , "Intensity Factor = 0x%x",sWeldRecipeParams.IntensityFactor);
		break;
	case 0x06:
		sprintf(printVar , "PartReflectivityFactor = 0x%x",sWeldRecipeParams.PartReflectivityFactor);
		break;
	case 0x07:
		sprintf(printVar , "Tool Reflectivity Factor = 0x%x",sWeldRecipeParams.ToolReflectivityFactor);
		break;
	default:
		sprintf(printVar , "Invalid Sub Index");
		break ;
	}
#endif
}
...
/////////////////////////////////////////////////////////////////////////////////////////
/**
 \param    subIndex      subIndex for which SDO request received.

 \brief    Function prints onto console the content at sub Index of the Weld Recipe object
 	 	 	 for which SDO Upload / Download request received.
*////////////////////////////////////////////////////////////////////////////////////////
void SDO_ConsoleLogging(ESS_UINT8 subIndex , ESS_UINT8 completeAccess )
{

	Uint8 ind = 0 ;
	ESS_INT8 weldRecipeFieldPrint[MAX_PRINT_WELD_RECIPE_FIELD_SIZE] ;


	/* Do not print in case of completeAccess not set and subIndex not in range and
	 * completeAccess is set and subIndex not equal to 0 or 1 (erraneous access, slave returns response with error code)*/
	if(((FALSE == completeAccess) && (subIndex > 7)) || ((TRUE == completeAccess) && (subIndex > 1)))
	{
		return ;
	}
	else
	{
		if(FALSE == completeAccess)
		{
			memset(weldRecipeFieldPrint , '\0' , MAX_PRINT_WELD_RECIPE_FIELD_SIZE) ;
			weldRecipeFieldConsoleLog(weldRecipeFieldPrint , subIndex);
			printf("\nEcSlave: 0x210A:%u - %s ", subIndex, weldRecipeFieldPrint);
		}
		else if(TRUE == completeAccess)
		{
			for(ind = 1 ; ind <= 7 ; ind++)
			{
				memset(weldRecipeFieldPrint , '\0' , MAX_PRINT_WELD_RECIPE_FIELD_SIZE) ;
				weldRecipeFieldConsoleLog(weldRecipeFieldPrint , ind);
				printf("\nEcSlave:  0x210A:%u - %s ",ind, weldRecipeFieldPrint);
			}
		}
	}
}
