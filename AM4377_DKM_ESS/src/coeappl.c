...
#if !TEST_APPLICATION
#warning "Backup functions need to be implemented"
/* the following functions has to be defined microcontroller specific */
 ...
 */
ESS_UINT32 u32Devicetype = (DEVICE_PROFILE_TYPE);
 ...
 */
OBJCONST ESS_UCHAR OBJMEM aName0x1000[] = "Device type";
 ...
 */
ESS_UINT16 u16ErrorRegister = 0x0;
 ...
 */
OBJCONST ESS_UCHAR OBJMEM aName0x1001[] = "Error register";
 ...
 */
ESS_CHAR acDevicename[] = DEVICE_NAME;
 ...
 */
OBJCONST ESS_UCHAR OBJMEM aName0x1008[] = "Device name";
 ...
 */
ESS_CHAR acHardwareversion[] = DEVICE_HW_VERSION;
 ...
 */
OBJCONST ESS_UCHAR OBJMEM aName0x1009[] = "Hardware version";
 ...
 */
ESS_CHAR acSoftwareversion[] = DEVICE_SW_VERSION;
 ...
 * \brief 0x1009 (Hardware version) object name
 */
OBJCONST ESS_UCHAR OBJMEM aName0x100A[] = "Software version";
...
typedef struct OBJ_STRUCT_PACKED_START {
   ...
   ESS_UINT32   aEntries[1]; /**< \brief Entry buffer*/
 ...
 */
OBJCONST ESS_UCHAR OBJMEM aName0x1010[] = "Store parameters";
...
ESS_UINT8 Write0x1010( ESS_UINT16 index, ESS_UINT8 subindex, ESS_UINT32 dataSize, ESS_UINT16 MBXMEM * pData, ESS_UINT8 bCompleteAccess )
{
   if ( bCompleteAccess )
 ...
 */
OBJCONST ESS_UCHAR OBJMEM aName0x1011[] = "Restore default parameters";
...
*/
ESS_UINT8 Write0x1011( ESS_UINT16 index, ESS_UINT8 subindex, ESS_UINT32 dataSize, ESS_UINT16 MBXMEM * pData, ESS_UINT8 bCompleteAccess )
{
   if ( bCompleteAccess )
...
typedef struct OBJ_STRUCT_PACKED_START {
   ESS_UINT16   u16SubIndex0; /**< \brief SubIndex 0*/
   ESS_UINT32   u32VendorID; /**< \brief EtherCAT vendor ID */
   ESS_UINT32   u32Productcode; /**< \brief Product code*/
   ESS_UINT32   u32Revision; /**< \brief Revision number*/
   ESS_UINT32   u32Serialnumber; /**< \brief Serial number*/
} OBJ_STRUCT_PACKED_END
TOBJ1018;
 ...
 */
OBJCONST ESS_UCHAR OBJMEM aName0x1018[] = "Identity\000Vendor ID\000Product code\000Revision\000Serial number\000\377";
...
typedef struct OBJ_STRUCT_PACKED_START {
   ESS_UINT16   u16SubIndex0; /**< \brief SubIndex 0*/
   ESS_UINT32   u32Checksum; /**< \brief Checksum*/
} OBJ_STRUCT_PACKED_END
TOBJ10F0;
 ...
 */
OBJCONST ESS_UCHAR OBJMEM aName0x10F0[] = "Backup parameter handling\000Checksum\000\377";
#endif // BACKUP_PARAMETER_SUPPORTED
 ...
 */   
OBJCONST ESS_UCHAR OBJMEM aName0x10F1[] = "Error Settings\000Local Error Reaction\000Sync Error Counter Limit\000\377";
...
typedef struct OBJ_STRUCT_PACKED_START {
   ESS_UINT16   u16SubIndex0; /**< \brief SubIndex 0*/
   ESS_UINT8   aEntries[4]; /**< \brief Entry buffer (one for each SyncManager)*/
} OBJ_STRUCT_PACKED_END
TOBJ1C00;
 ...
 */
OBJCONST ESS_UCHAR OBJMEM aName0x1C00[] = "Sync manager type";
 ...
 */
OBJCONST ESS_UCHAR OBJMEM aName0x1C33[] = "SM input parameter\000Synchronization Type\000Cycle Time\000\000Synchronization Types supported\000Minimum Cycle Time\000Calc and Copy Time\000\000Get Cycle Time\000Delay Time\000Sync0 Cycle Time\000SM-Event Missed\000Cycle Time Too Small\000Shift Time Too Short\000\000\000\000\000\000\000\000\000\000\000\000\000\000\000\000\000\000\000Sync Error\000\377";
#endif
...
void InitDefaultEntries(void)
{
...
#endif
    {
        if (pObjEntry->NonVolatileOffset != 0)
        {
            ...
            ESS_UINT32 size = OBJ_GetObjectLength( pObjEntry->Index, 0, pObjEntry, 1);
            /* call the microcontroller specific function to store the data in the non-volatile memory */
            EE_WriteWordsToNonVolatileMemory(pObjEntry->NonVolatileOffset,(ESS_UINT16 *) pObjEntry->pVarPtr, (ESS_UINT16) BYTE2WORD(size));
        }
#if !STATIC_OBJECT_DIC
        ...
        if (pObjEntry->NonVolatileOffset != 0)
        {
            ...
            for (i = 0; i <= maxSubindex; i++)
            {
                ...
                if (pEntry->ObjAccess & OBJACCESS_BACKUP)
                {
                    ...
                    ESS_UINT16 wordOffset = OBJ_GetEntryOffset(i, pObjEntry) >> 4;

                    ESS_UINT16 wordSize = BIT2WORD(pEntry->BitLength);

                    /* call the microcontroller specific function to load the data from the non-volatile memory */
                    EE_ReadWordsFromNonVolatileMemory(&((ESS_UINT16 *) pObjEntry->pVarPtr)[wordOffset], pObjEntry->NonVolatileOffset+wordOffset, wordSize);
                }
            }
        }
#if !STATIC_OBJECT_DIC
        ...
        if (pObjEntry->NonVolatileOffset != 0)
        {
            ...
            for (i = 0; i <= maxSubindex; i++)
            {
                ...
                if (pEntry->ObjAccess & OBJACCESS_BACKUP)
                {
                    ...
                    sBackupparameterhandling.u32Checksum = EE_WriteWordsToNonVolatileMemory(pObjEntry->NonVolatileOffset+wordOffset, &((ESS_UINT16 *) pObjEntry->pVarPtr)[wordOffset], wordSize);
                }
            }
        }
#if !STATIC_OBJECT_DIC
...
void COE_WriteBackupEntry(ESS_UINT8 subindex, OBJCONST TOBJECT OBJMEM * pObjEntry)
{
    /* get the offset of the entry in the object's variable */
    ESS_UINT16 wordOffset = OBJ_GetEntryOffset(subindex, pObjEntry) >> 4;
    ...
    /* call the microcontroller specific function to store the data in the non-volatile memory
       and store the checksum in backup parameter checksum object */
    sBackupparameterhandling.u32Checksum = EE_WriteWordsToNonVolatileMemory(pObjEntry->NonVolatileOffset+wordOffset, &((ESS_UINT16 MBXMEM *) pObjEntry->pVarPtr)[wordOffset], wordSize);
}
#endif // BACKUP_PARAMETER_SUPPORTED
...
/*ECATCHANGE_END(V5.11) ECAT4*/
    ...
    {
    ESS_UINT16 result = COE_ObjDictionaryInit();
    if(result != 0)
    {
 ...
 *////////////////////////////////////////////////////////////////////////////////////////
ESS_UINT16 COE_AddObjectToDic(TOBJECT OBJMEM * pNewObjEntry)
{
...
*////////////////////////////////////////////////////////////////////////////////////////
void COE_RemoveDicEntry(ESS_UINT16 index)
{
    TOBJECT    OBJMEM * pDicEntry = ObjDicList;
...
ESS_UINT16 AddObjectsToObjDictionary(TOBJECT OBJMEM * pObjEntry)
{
    ESS_UINT16 result = 0;
...
*////////////////////////////////////////////////////////////////////////////////////////
ESS_UINT16 COE_ObjDictionaryInit(void)
{
    ESS_UINT16 result = 0;
...
void COE_Main(void)
{
    ...
    if(pSdoPendFunc != NULL)
    {
        abort = pSdoPendFunc(u16StoreIndex,u8StoreSubindex,u32StoreDataSize,pStoreData,bStoreCompleteAccess);
         ...
         if (0x0 == abort)
         {
        	 if(u16StoreIndex == WELD_RECIPE_OBJ_IND)
                        	{
                        	       SDO_ConsoleLogging(u8StoreSubindex ,bStoreCompleteAccess );
                        	}
                        }
#endif
