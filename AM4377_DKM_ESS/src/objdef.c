...
#define CHANGE_IN_FIRMWARE_UPGRADE_OD  	6
...
------
---------------------------------------------------------------------------------------*/
const ESS_UINT16 cBitMask[16] = {0x0000,0x0001,0x0003,0x0007,0x000F,0x001F,0x003F,0x007F,0x00FF,0x01FF,0x03FF,0x07FF,0x0FFF,0x1FFF,0x3FFF,0x7FFF};
/*---------------------------------------------------------------------------------------
------
------    Functions
------
...
OBJCONST TOBJECT OBJMEM *  OBJ_GetObjectHandle( ESS_UINT16 index )
{
	//printf("\nOBJ_GetObjectHandle: index: 0x%x\n",index);
...
#endif
    {
        ...
        {
        	//printf("OBJ_GetHandle- Return value : 0x%lx\n",pObjEntry);
            return pObjEntry;
        }
#if !STATIC_OBJECT_DIC
...
*////////////////////////////////////////////////////////////////////////////////////////
ESS_UINT32 OBJ_GetObjectLength( ESS_UINT16 index, ESS_UINT8 subindex, OBJCONST TOBJECT OBJMEM * pObjEntry, ESS_UINT8 bCompleteAccess)
{
	//printf("\nOBJ_GetObjectLength: index: 0x%x, subIndex: 0x%x, pObjEntry: 0x%lx, bCompleteAccess: 0x%x\n",index, subindex, pObjEntry, bCompleteAccess);
    /* get the information of ObjCode and MaxSubindex in local variables to support different types of microcontroller */
    ESS_UINT8 objCode = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_OBJCODEMASK) >> OBJFLAGS_OBJCODESHIFT;
#if COMPLETE_ACCESS_SUPPORTED
    ESS_UINT8 maxSubindex = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_MAXSUBINDEXMASK) >> OBJFLAGS_MAXSUBINDEXSHIFT;
    ESS_UINT32 size = 0;
    ...
    if ( bCompleteAccess )
    {
            ...
            )
        {
            ...
            if(index == 0x8000)
            {
                ...
                if ( subindex == 0 )
                {
                    /* add size for subindex 0 (is transmitted as ESS_UINT16) */
                    size += 2;
                }
                return size;
            }
            ...
            /* we have to get the maxSubindex from the actual value of subindex 0,
                which is stored as ESS_UINT16 at the beginning of the object's variable */
            maxSubindex = (ESS_UINT8) ((ESS_UINT16 MBXMEM *) (pObjEntry->pVarPtr))[0];
...
/*ECATCHANGE_END(V5.11) TEST2*/
        ...
        {
            /* for single access subindex 0 is transmitted as ESS_UINT8 */
            return 1;
        }
        else if((objCode == OBJCODE_ARR)
#if USE_SINGLE_PDO_MAPPING_ENTRY_DESCR
...
void OBJ_CopyNumberToString(ESS_UCHAR MBXMEM *pStr, ESS_UINT8 Number)
{
    ESS_UINT8 Modulo;
...
ESS_UINT16    OBJ_GetNoOfObjects(ESS_UINT8 listType)
{
    ...
    ESS_UINT16 listFlags = 0x0020 << listType;
...
#endif
    {
        ...
        if ( pObjEntry->Index >= 0x1000 )
        {
            ...
            if ( t )
            {
                ...
                {
                    if ( OBJ_GetEntryDesc(pObjEntry,(ESS_UINT8) i)->ObjAccess & listFlags )
                        t = 0;
                    i++;
                }
            }
...
*////////////////////////////////////////////////////////////////////////////////////////
ESS_UINT16    OBJ_GetObjectList(ESS_UINT16 listType, ESS_UINT16 *pIndex, ESS_UINT16 size, ESS_UINT16 MBXMEM *pData,ESS_UINT8 *pAbort)
{
    /* the variable listFlags contains the mask used for the ObjAccess in the Entry-Desc
       see the structure TSDOINFOENTRYDESC in sdoserv.h, listType = 0 indicates that
       all objects has to be counted */
    ...
    if ( pObjEntry != NULL )
    {
...
#endif
        {
            ...
            if ( pObjEntry->Index >= 0x1000 )
            {
                /* ESS_UINT8 was changed to ESS_UINT16 */
                ESS_UINT16 t = listType;
                if ( t )
                {
                    ESS_UINT8 maxSubindex = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_MAXSUBINDEXMASK) >> OBJFLAGS_MAXSUBINDEXSHIFT;
                    ESS_UINT8 i = 0;
...
ESS_UINT16 OBJ_GetDesc( ESS_UINT16 index, ESS_UINT8 subindex, OBJCONST TOBJECT OBJMEM * pObjEntry, ESS_UINT16 MBXMEM * pData )
{
    ESS_UINT16 strSize = 0;
    OBJCONST ESS_UCHAR OBJMEM * pDesc = (OBJCONST ESS_UCHAR OBJMEM *) pObjEntry->pName;
/* get the information of ObjCode and MaxSubindex in local variables to support different types of microcontroller */
    ESS_UINT8 objCode = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_OBJCODEMASK) >> OBJFLAGS_OBJCODESHIFT;
    ...
    if ( (subindex == 0) || (objCode == OBJCODE_VAR) )
    {
        ...
        strSize = OBJSTRLEN( (OBJCONST ESS_CHAR OBJMEM *) pDesc );
    ...
    else
    {
        if ( objCode == OBJCODE_REC )
        {
            ...
            if(IS_TEST_OBJECT(index) && subindex > 0)
            {
                ...
                strSize = OBJSTRLEN( (OBJCONST ESS_CHAR OBJMEM *) pTestObj->paEntries[(subindex - 1)]->pName );
                if ( pData && strSize )
                    OBJTOMBXSTRCPY( pData, pTestObj->paEntries[(subindex - 1)]->pName, strSize );
            }
            else if(index == 0x8000) //slave configuration object
            {
                strSize = OBJSTRLEN((OBJCONST ESS_CHAR OBJMEM *) aSlaveConfig[(subindex-1)].pName);
            ...
            ESS_UINT16 i = 1;
            ESS_UINT16 tmpSubindex = subindex;
            ...
            OBJCONST ESS_UCHAR OBJMEM * pSubDesc = (OBJCONST ESS_UCHAR OBJMEM *) OBJGETNEXTSTR( pDesc );
            while (( i <= tmpSubindex )
                &&( pSubDesc[0] != 0xFF && pSubDesc[0] != 0xFE ))
            {
                if ( i == tmpSubindex )
                {
                    strSize = OBJSTRLEN( (OBJCONST ESS_CHAR OBJMEM *) pSubDesc );
                    if ( pData && strSize )
...
/* ECATCHANGE_END(V5.11) SDO6*/
                ...
                {
                    i++;
                    pSubDesc = (OBJCONST ESS_UCHAR OBJMEM *) OBJGETNEXTSTR( pSubDesc );
                }
            }
            }
            }
...
OBJCONST TSDOINFOENTRYDESC OBJMEM * OBJ_GetEntryDesc(OBJCONST TOBJECT OBJMEM * pObjEntry, ESS_UINT8 Subindex)
{
...
ESS_UINT16 OBJ_GetEntryOffset(ESS_UINT8 subindex, OBJCONST TOBJECT OBJMEM * pObjEntry)
{
    ESS_UINT16 i;
    /* bitOffset will be initialized with the bit offset of subindex 1 */
/*ECATCHANGE_START(V5.11) OBJ1*/
    ESS_UINT16 bitOffset = 0;
    ESS_UINT8 objCode = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_OBJCODEMASK) >> OBJFLAGS_OBJCODESHIFT;
    OBJCONST TSDOINFOENTRYDESC OBJMEM *pEntry;

    if(subindex > 0)
    {
        /*subindex 1 has an offset of 16Bit (even if Si0 is only an ESS_UINT8) */
        bitOffset +=16;
    }
...
*////////////////////////////////////////////////////////////////////////////////////////
ESS_UINT8 CheckSyncTypeValue(ESS_UINT16 index, ESS_UINT16 NewSyncType)
{
...
ESS_UINT8 OBJ_Read( ESS_UINT16 index, ESS_UINT8 subindex, ESS_UINT32 objSize, OBJCONST TOBJECT OBJMEM * pObjEntry, ESS_UINT16 MBXMEM * pData, ESS_UINT8 bCompleteAccess )
{
	//printf("\nOBJ_Read_OBJ: index: 0x%x, subIndex: 0x%x, objSize: 0x%lx, pObjEntry: 0x%lx, bCompleteAccess: 0x%x\n",index, subindex, objSize, pObjEntry, bCompleteAccess);
    ESS_UINT16 i = subindex;
    /* get the information of ObjCode and MaxSubindex in local variables to support different types of microcontroller */
    ESS_UINT8 objCode = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_OBJCODEMASK) >> OBJFLAGS_OBJCODESHIFT;
    ESS_UINT16 maxSubindex = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_MAXSUBINDEXMASK) >> OBJFLAGS_MAXSUBINDEXSHIFT;
    OBJCONST TSDOINFOENTRYDESC OBJMEM *pEntry;
#if COMPLETE_ACCESS_SUPPORTED
    ...
    ESS_UINT16 lastSubindex = subindex;
#endif

    if ( objCode != OBJCODE_VAR && index >= 0x1000 )
    {
        /* if the object is an array or record we have to get the maxSubindex from the
        actual value of subindex 0, which is stored as ESS_UINT16 at the beginning of the
        object's variable */
        maxSubindex = (*((ESS_UINT16 *) (pObjEntry->pVarPtr))) & 0x00FF;
    ...
    if ( bCompleteAccess )
    {
        ...
        else
        {
            ...
            pEntry = OBJ_GetEntryDesc(pObjEntry, (ESS_UINT8)i);
            ...
            if(pEntry->ObjAccess != 0x0)
            {
                ...
                if ( ((ESS_UINT8) ((pEntry->ObjAccess & ACCESS_READ)<<1)) < (nAlStatus & STATE_MASK) )
                {
        ...
        else if ( index < 0x1000 && subindex != 0 )
        {
            ...
            ESS_UINT16 size;
            ESS_UINT16 MBXMEM *pVarPtr = (ESS_UINT16 MBXMEM *) pObjEntry->pVarPtr;
            ESS_CHAR **p;
            ...
            p = (ESS_CHAR **) pVarPtr;
            pVarPtr = (ESS_UINT16 MBXMEM *)p[subindex-1];

#if MBX_16BIT_ACCESS
            if((((ESS_UINT16)pVarPtr) & 0x1) == 0x1)
            {
...
/*ECATCHANGE_START(V5.11) SDO3*/
                pVarPtr = (ESS_UINT16 MBXMEM *)(((MEM_ADDR)pVarPtr)& ~(MEM_ADDR)0x1);
        ...
        if(index == 0x3004 && subindex == 14 && bCompleteAccess == FALSE)
        {
...
#endif
            {
                ...
                else if ( index >= 0x1000 )
                {
                    ...
                    bitOffset = OBJ_GetEntryOffset((ESS_UINT8)i, pObjEntry);
                }
                ...
                if ( ((ESS_UINT8) ((pEntry->ObjAccess & ACCESS_READ)<<1)) >= (nAlStatus & STATE_MASK) )
                {
                        ...
                        || (bCompleteAccess && i >= subindex) )       /* complete access and entry should be read */
                    {
                        ...
                        if ( i == 0 && objCode != OBJCODE_VAR )
                        {
                            ...
                            if(IS_TEST_ACTIVE(SDOFullObjectCompleteAccess_Test) && bCompleteAccess)
                            {
                                pData[0] = SWAPWORD((*((ESS_UINT16 *) (pObjEntry->pVarPtr))) & 0x00FF);
                            }
                            else
#endif
                            {
                                pData[0] = SWAPWORD((ESS_UINT16)maxSubindex);
                            }

                            /* we increment the destination pointer by 2 because the subindex 0 will be
                            transmitted as ESS_UINT16 for a complete access */
                            pData++;
                        }
                        else
                        {
                            ESS_UINT16 dataType = pEntry->DataType;
                            ...
                            case DEFTYPE_NULL:
                                if(bCompleteAccess)
                                {
                                    ...
                                    {
                                        /* we have reached the ESS_UINT16 border */
                                        pData++;
                                    }
...
ESS_UINT8 OBJ_Write( ESS_UINT16 index, ESS_UINT8 subindex, ESS_UINT32 dataSize, OBJCONST TOBJECT OBJMEM * pObjEntry, ESS_UINT16 MBXMEM * pData, ESS_UINT8 bCompleteAccess )
{
	MSG_Q_ID msgQId_f;
	char msg[2];
	//printf("\nOBJ_Write_OBJ: index: 0x%x, subIndex: 0x%x, objSize: 0x%lx, pObjEntry: 0x%lx, bCompleteAccess: 0x%x\n",index, subindex, dataSize, pObjEntry, bCompleteAccess);
    ESS_UINT16 i = subindex;
    /* get the information of ObjCode and MaxSubindex in local variables to support different types of microcontroller */
    ESS_UINT8 objCode = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_OBJCODEMASK) >> OBJFLAGS_OBJCODESHIFT;
    ESS_UINT16 maxSubindex = 0;
    ESS_UINT16 maxConfiguredSubindex = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_MAXSUBINDEXMASK) >> OBJFLAGS_MAXSUBINDEXSHIFT; //required to check if value for Subindex0 is valid
    OBJCONST TSDOINFOENTRYDESC OBJMEM *pEntry;
    ESS_BOOL bClearSubindex0Required = FALSE;
#if COMPLETE_ACCESS_SUPPORTED
    /* lastSubindex is used for complete access to make loop over the requested entries
       to be read, we initialize this variable with the requested subindex that only
       one loop will be done for a single access */
    ...
    if ( bCompleteAccess )
    {
        ...
        if ((subindex == 0) && (dataSize > 0))
        {
            ...
            maxSubindex = (ESS_UINT8) SWAPWORD(pData[0]);
        }
    ...
    else
    {
        ...
        /* check if we have write access (bits 3-5 (PREOP, SAFEOP, OP) of ObjAccess)
           by comparing with the actual state (bits 1-3 (PREOP, SAFEOP, OP) of AL Status) */
        if ( ((ESS_UINT8) ((pEntry->ObjAccess & ACCESS_WRITE) >> 2)) < (nAlStatus & STATE_MASK) )
        {
        ...
        )
    {
        ...
        ESS_UINT16 Subindex0 = (*(ESS_UINT16 *)pObjEntry->pVarPtr) & 0x00FF;
        if(Subindex0 != 0x00)
    ...
    {
        ESS_UINT8 bWritten = 0;
        ESS_UINT8 result = ABORTIDX_READ_ONLY_ENTRY;
            ...
            if ( ((ESS_UINT8)((pEntry->ObjAccess & ACCESS_WRITE) >> 2)) >= (nAlStatus & STATE_MASK) )
            {
                  ...
                  || (bCompleteAccess && i >= subindex) )       /* complete access and entry should be read */
                {
                    ...
                    if (i == 0 && objCode != OBJCODE_VAR)
                    {
                        ...
                        ESS_UINT8 NewSubindex0 = (ESS_UINT8) SWAPWORD(pData[0]);
...
/*ECATCHANGE_END(V5.11) SDO1*/
                            ...
                            /* in this example the objects are defined in that way,
                            that the 16 bit type are always starting at an exact WORD offset */
                            ESS_UINT16 u16NewData = SWAPWORD(pData[0]);
...
/*ECATCHANGE_END(V5.11) SDO2*/
                    ...
                    if (pEntry->ObjAccess & OBJACCESS_BACKUP)
                    {
                        /* if the backup has to be stored immediately, we have to check, if the non-volatile memory
                           has to be updated */
                        COE_WriteBackupEntry((ESS_UINT8)i, pObjEntry);
                    }
#endif
                }
            }
...
/*ECATCHANGE_END(V5.11) SDO9*/
		...
		if (index == 0x4020 )
		{
			kprintf("\nSending Notification FromOBJ_Write to FirmwareUpgrade task  \n");
			msg[0] = CHANGE_IN_FIRMWARE_UPGRADE_OD;
			msg[1] = 0;
			msgQId_f = msgQOpen("/Fw_Msgq_Notify",0,0,0,0,0);
			if(msgQSend(msgQId_f,msg,2,0,0) != OK)
			{
					kprintf("\nSending Notification FromOBJ_Write to FirmwareUpgrade task Failed \n");
			}
			msgQClose(msgQId_f);
		}
	#endif
		
	#if POWER_CONTROLLER_BUILD
		if (index == 0x2020 )
		{
			msg[0] = CHANGE_IN_FIRMWARE_UPGRADE_OD;
			msg[1] = 0;
