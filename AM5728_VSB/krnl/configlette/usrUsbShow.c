...
/*
 * Copyright (c) 2007, 2008, 2010-2012, 2014-2016 Wind River Systems, Inc.
 *
...
--------------------
07dec16,whu  Add the declaration of uLength if component 
             INCLUDE_USB_GEN2_PRINTER_INIT is added (V7CO-434)
04mar16,whu  Fix the incorrect printing of the PID/VID pair when lsusb (V7CON-375)
12dec15,jzl  clean build warning(V7CON-294)
17apr15,j_x  Add super speed definition (V7CON-252)
02dec14,lan  Remove the compiler warning (V7CON-192)
20may14,wyy  Change portStatus in getPortStatus() to 32bit (VXW6-82588) 
28feb12,ljg  Add declaration of usrUsbWaitTimeOutValueGet (WIND00330641)
11aug11,j_x  Use USB ossLib to allocate buffer (WIND00266669) 
23feb11,w_x  Fix duplicates by usrDepend.c and usrUsbInit.c (WIND00247788) 
24jun10,m_y  Add valid check after allocate resource,
             replace LOCALASSERT to if judgement (WIND00183499)
05jul10,ghs  Fix conflict symbol defines(WIND00221550)
24may10,m_y  Add parameter valid check,
             modify URB timeout value for usb robustness (WIND00183499)
18may10,w_x  Move some structures from usb2MscP.h to usb2Msc.h
27apr10,j_x  Add GEN2 class driver show (WIND00184395)
18mar10,j_x  Port to VxWorks6.9 (WIND00184542)
32Jan08,jrp  Fixed endian problems.
06Jul07,jrp  Created.
*/
...
LOCAL STATUS searchHub
    ...
    if (usbHubPortCntGet(pHub->uDeviceHandle,&numPorts ) != OK)
        {
        DEBUGMSG("\n usbHubPortCntGet failed %s %d",(_Vx_usr_arg_t)__FILE__,__LINE__,
                0, 0, 0, 0);
        return(ERROR);
        }
...
LOCAL STATUS getPortStatus
    ...
    while (NULL != pBusList)
        ...
        DEBUGMSG ("\n Root hub - 0x%X  devid - 0x%x",(_Vx_usr_arg_t)pRootHub,
...
LOCAL VOID printPortStatus
    ...
    if (pDevDescr->bcdUSB >= 0x0300)
        {
        if ((portStatus & M_PORT_HIGH_SPEED) == 0)
            fprintf(fp,"SUPER SPEED");
        else
            fprintf(fp,"Unknown SPEED");
        }
        
    else if (portStatus & M_PORT_LOW_SPEED)
        {
                ...
                if (verboseLevel > 1)
                     ...
                     */
                     ...
                     if (nodeType != USBHUB_NODETYPE_HUB)
                         ...
                         printPortStatus(fp,indent,portStatus,pDevDescr);
                         }
                     }
                /*
...
LOCAL STATUS USBShow
    ...
    for (busIndex = 0; busIndex < busCount; busIndex++)
        ...
        if (usbHubGetRootHubHandle(busIndex, &rootHub) != OK)
            {
            logMsg("Unexpected failure %s %d",(_Vx_usr_arg_t)__FILE__,__LINE__,0,0,0,0);
            return (ERROR);
            }
        if (level > 0)
            {
