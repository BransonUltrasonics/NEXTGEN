...
typedef struct {   
 ...
 typedef struct pruEscDevInfo
   ...
   #define AM38XX_PWM_BAR(p)       (((PWMGEN *)vxbDevSoftcGet(p))->regBase)
   #define AM38XX_PWM_HANDLE(p)    ((PWMGEN *)vxbDevSoftcGet(p))->regHandle

  #define CSR_READ_4(pDev, addr)                          \
      vxbRead32 (AM38XX_PWM_HANDLE(pDev),                 \
          (unsigned int *)((char *)AM38XX_PWM_BAR(pDev) + addr))

  #define CSR_WRITE_4(pDev, addr, data)                   \
      vxbWrite32 (AM38XX_PWM_HANDLE(pDev),                \
          (unsigned int *)((char *)AM38XX_PWM_BAR(pDev) + addr), data)

  #define CSR_READ_2(pDev, addr)                          \
      vxbRead16 (AM38XX_PWM_HANDLE(pDev),                 \
          (UINT16 *)((char *)AM38XX_PWM_BAR(pDev) + addr))

  #define CSR_WRITE_2(pDev, addr, data)                   \
      vxbWrite16 (AM38XX_PWM_HANDLE(pDev),                \
          (unsigned int *)((char *)AM38XX_PWM_BAR(pDev) + addr), data)
...
------------------------------------------------------------------------------*/
void Drv_PWM_Init(PWMGEN_handle);
void Drv_PWM_Update(PWMGEN_handle);
//void Drv_PWM_ClearInt(PRUICSS_Handle pruIcssHandle);


#define EPWMCLK_EN   		0x100
#define EPWMCLKSTOP_REQ   	0x200
#define EQEPCLK_EN   		0x10
#define EQEPCLKSTOP_REQ   	0x20
#define ECAPCLK_EN   		0x1
#define ECAPCLKSTOP_REQ   	0x2

#define REG_IDVER			0x0
#define REG_SYSCONFIG		0x4
#define REG_CLKCONFIG		0x8
#define REG_CLKSTATUS		0xC
 ...
 */
void EPWMTbTimebaseClkCfg(VXB_DEV_ID  pDev,uint32_t baseAddr,
                          uint32_t tbClk,
                          uint32_t moduleClk);
 ...
 */
void EPWMTbPwmFreqCfg(VXB_DEV_ID  pDev,uint32_t baseAddr,
                      uint32_t tbClk,
                      uint32_t pwmFreq,
                      uint32_t counterDir,
