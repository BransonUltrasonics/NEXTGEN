...
--------------------
01g,02may05,job  vxWorks-specific definition of SNMP_DH_RAND moves here
01f,06oct04,job  draw processor alignment from vxWorks header files
01e,26nov03,ps  copyright update
01d,05nov03,ps  merge from latest on ickthwa_devel_branch.nh
01c,19apr98,ann changed ENVOY_SNMP_INIT_LOCK macro
01b,04jun96,rjc changed #if defined to #if
...
#define SNMP_validate_community(pkt, src, dst) \
    snmpIoCommunityValidate (pkt, src, dst)
...
#define SNMP_release_private(pkt)  snmpHookPrivRelease (pkt)
...
#define SNMP_validate_set_pdu(pkt) snmpHookSetPduValidate (pkt)
...
#define SNMP_user_pre_set(pkt) snmpHookPreSet(pkt)
...
#define SNMP_user_post_set   snmpHookPostSet
...
#define SNMP_user_set_failed           snmpHookSetFailed
