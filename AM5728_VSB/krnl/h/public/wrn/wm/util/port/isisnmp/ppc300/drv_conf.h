/* @(#) pSOSystem :$RCSfile: drv_conf.h,v $ $Revision: 1.1.1.1 $ (&U&) $Date: 2001/11/05 17:47:11 $  */
...
/*--------------------------------------------------------------------*/ 
    ...
    InstallDriver(SC_DEV_MEMLOG, MemLogInit, NULLF,
                  ...
                  MemLogCntrl, 0, 0, IO_CHAR_DEV|SC_AUTOINIT);
    PsosNumDntEnts++;
#endif
...
/*---------------------------------------------------------------------*/
/* For pSOSim only - Install the pSOSim Console driver                 */
/*---------------------------------------------------------------------*/
#if (DEV_SIMCONSOLE)
    #if(SC_DEV_SIMCONSOLE > SC_DEVMAX)
        #error "SC_DEV_SIMCONSOLE cannot be > SC_DEVMAX"
    #endif
    InstallDriver(SC_DEV_SIMCONSOLE, SimConsoleInit, NULLF,
                  NULLF, SimConsoleRead, SimConsoleWrite,
                  0, 0, 0, IO_CHAR_DEV | SC_AUTOINIT);
    PsosNumDntEnts ++;
#endif
...
#include "drv_conf.idl"
    ...
    /*     1. Maximum NI interface number                          */
    /*     2. Maximum number of streams per interface              */
    /*-------------------------------------------------------------*/
    DlpiInit(dlpi_nicount, SE_DLPI_MAXSTR_PERNI);
...
/*-----------------------------------------------------------------*/
/*  pSOSim Console driver                                          */
/*-----------------------------------------------------------------*/
#if SC_DEV_SIMCONSOLE
    dnt_add("simconsole", DEV_SIMCONSOLE);
#endif
...
void SetUpNI(void)
{
void (*InstallNi_func)( int (*entry)(ULONG, union nientry *), int, int,
                      int, int, int, int );

    InstallNi_func = InstallNi;
...
/*---------------------------------------------------------------------*/
    ...
    if (SysVars.Lan1)
        {
        register int IPaddr;
        register int (*NiLanPtr)(ULONG, union nientry *);

        #if (SC_DEV_DLPI && SE_SHARE_NI)
            NiLanPtr = (int (*)(ULONG, union nientry *))DlpiEnet;
        #else
            NiLanPtr = (int (*)(ULONG, union nientry *))BSP_LAN1_ENTRY;
        #endif
        ...
        if (SysVars.Lan1IP == USE_RARP)
            ...
            InstallNi_func(
            NiLanPtr, IPaddr, BSP_LAN1_MTU, BSP_LAN1_HWALEN,
...
/*---------------------------------------------------------------------*/
    ...
    if (SysVars.Nism)
        {
        InstallNi_func((int (*)(unsigned long, union nientry *))NiSmem,
                      SysVars.NismIP, NISM_MTU, 4, 0x8002,
