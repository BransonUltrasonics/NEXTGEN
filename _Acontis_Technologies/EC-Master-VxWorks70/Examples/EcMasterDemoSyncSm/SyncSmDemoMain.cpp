...
EC_T_DWORD EnableRealtimeEnvironment(EC_T_VOID)
{
    ...
    if (nRetval != 0)
    {
        OsPrintf("EnableRealtimeEnvironment: rt_task_set_mode returned error %d\n", nRetval);
...
{
    OsDbgMsg("Syntax:\n");
    OsDbgMsg("EcMasterDemoSyncSm [-f ENI-FileName] [-t time] [-b time] [-a affinity] [-v lvl] [-perf] [-log Prefix]");
#if (defined AUXCLOCK_SUPPORTED)
...
*/
#if (defined VXWORKS)
extern "C" int syncsmDemo(char* lpCmdLine)
...
TLR_RESULT syncsmDemo(TLR_VOID FAR* pvInit)
#elif (defined STARTERWARE_NOOS)
extern "C" int EcMasterDemoSyncSm(int nArgc, char* ppArgv[])
#elif (defined RTAI)
extern "C" void EcMasterDemoSyncSmMain(long int t)
...
extern "C" int EcMasterDemoSyncSm(void)
...
#endif
{
    ...
    UINT32 dwLength = 0;
        ...
        if ('\0' == szCommandLine[0])
        {
            ...
            if (RTE_SUCCESS != dwRes)
            {
                OsPrintf("Cannot read EtherCAT demo command line, (EcatShm.config)\n");
                OsPrintf("Please, enter command line (e.g. atemDemo -v 2 -i8255x 1 1):\n");
            }
        }
    }
#else
...
Exit:
    ...
    OsDbgMsg("EcMasterDemoSyncSm stop.\n");
#if !(defined VXWORKS)
